...
#else
	unsigned char dieday:3, inprison:1, mypic:1, nouse1:3;
#endif
	char exp_group;
	unsigned char extraexp1;
	char nouse[1];
...
char *cuserexp(int group, int exp);
char *cperf(int);
int countexp(struct userec *, int);
...
char * getuseridbynum(const int uid);
