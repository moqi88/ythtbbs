...
max_mailsize(struct userec *lookupuser)
{
	...
	//关闭信箱
	if(!(lookupuser->userdefine & DEF_SEEWELC1))
		return 0;
	
	//经验值等级
	exp = countexp(lookupuser, 2);
	for(lvl=0; exp > 5*lvl*lvl+15*lvl; lvl++ );

	//精华值
	exp2 = countexp(lookupuser, 1);

	if(lvl>=100 || exp2 >= 18900)
		maxsize = BASE_MAIL_HOLD * 10;
	else
		maxsize = BASE_MAIL_HOLD + 500 * (lvl/10);

	if(USERPERM(lookupuser, PERM_SYSOP)||USERPERM(lookupuser, PERM_SPECIAL1))
		maxsize = MAX_SYSOPMAIL_HOLD > maxsize ? MAX_SYSOPMAIL_HOLD : maxsize;
	else if( USERPERM(lookupuser, PERM_ARBITRATE) || USERPERM(lookupuser, PERM_SPECIAL4) 
		|| USERPERM(lookupuser, PERM_SPECIAL5) || USERPERM(lookupuser, PERM_SPECIAL6)
		|| USERPERM(lookupuser, PERM_BOARDS) || USERPERM(lookupuser, PERM_EXT_IDLE) )
		maxsize = 3000 > maxsize ? 3000 : maxsize;
	  ...
	  int sysmail)
{
	...
	if(!sysmail && !(lookupuser->userdefine & DEF_SEEWELC1))
		return -1;
...
{
	return _mail_buf(buf, size, userid, title, sender, 0);
}
...
int
bbs_sendmail(fname, title, receiver, sender, filter_ansi)
...
int filter_ansi;
{
	...
	if (invalidaddr(receiver))
		return -1;
	snprintf(sbuf, sizeof(sbuf), "/usr/lib/sendmail -f %s.bbs@%s '%s'",
		sender, MY_BBS_DOMAIN, receiver);
	fout = popen(sbuf, "w");
	fin = fopen(fname, "r");
	...
	while (fgets(b64_in, sizeof(b64_in), fin) != NULL) {
		if (NULL != (attach = checkbinaryattach(b64_in, fin, &len))) {
			...
			fprintf(fout, "Content-Transfer-Encoding: Base64\n\n");
			f_b64_ntop_init(tail_buf, &tail_len, &outlen);
			f_b64_ntop(fout, buf, len, tail_buf, &tail_len, &outlen);
			f_b64_ntop_fini(fout, tail_buf, &tail_len);
			fprintf(fout, "\n--" MY_MIME_BOUNDARY "\n");
			fprintf(fout, "Content-Type: text/plain; charset=gb2312\n");
			fprintf(fout, "Content-Transfer-Encoding: Base64\n\n");
		...
		if (filter_ansi)
			filteransi(b64_in);
		f_b64_ntop(fout, b64_in, strlen(b64_in), tail_buf, &tail_len, &outlen);
	}
	f_b64_ntop_fini(fout, tail_buf, &tail_len);
	fprintf(fout, "\n--" MY_MIME_BOUNDARY "--\n");
	fprintf(fout, ".\n");
...
int bbs_sendmail_noansi(fname, title, receiver, sender)
char *fname, *title, *receiver, *sender;
{
