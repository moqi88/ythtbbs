		   ...
		   const char *randstr)
{
	...
	sprintf(buf, "/usr/lib/sendmail -f Account.bbs@%s %s &>/dev/null",
	...
	fprintf(fout, "Reply-To: Account.bbs@%s\n", MY_BBS_DOMAIN);
	fprintf(fout, "From: Account.bbs@%s\n", MY_BBS_DOMAIN);
	fprintf(fout, "To: %s\n", udata->email);
	fprintf(fout, "Subject: @%s@[-%s-]%s mail check.\n", tmpu->userid,
		randstr, MY_BBS_ID);
	fprintf(fout, "X-Forwarded-By: Account \n");
...
int
sendInvitation(const char *inviter, const char *myname, const char *toemail, const char *toname,
	       const char *tmpfn)
{
	int code, t, retv;
	FILE *fout;
	...
	fprintf(fout, "%s 邀请您在 %s BBS 建立一个用户。"
		"这封邀请信在两周内有效。\n\n",
		myname, MY_BBS_NAME);
	...
	if (!strcmp(MY_BBS_ID, "yilubbs"))
		fprintf(fout,
			"-------------------------一路BBS简介--------------------------\n"
			"在2004年9月，我们曾经在岔路分别，如今，我们又慢慢聚集在一起。\n"
			"希望我们从头开始，一路携手前行。大火过后的荒原，只需要一阵雨水，\n"
			"我们便可生长。\n\n"
			"一路BBS站于2008年12月18日正式开站，我们得到了原一塌糊涂公开\n"
			"版面和精华区数据的授权，也将继承它的风格与规则。我们将努力维持\n"
			"一个良好的网上交流氛围，打造一个遵守规则的站点，依靠站规来处理\n"
			"站点的各类冲突。我们欢迎所有人、所有团体，在站规的限定下在本站\n"
			"宣传、交流、活动。\n\n"
			"----------------------------------------------------------------\n\n");
	fprintf(fout,
		"如果您想了解更多%s BBS 的概况，可以访问一下我们的 WWW 页面，\n"
		"        http://%s/\n", MY_BBS_NAME, MY_BBS_DOMAIN);
	fprintf(fout, "或者用 telnet 访问，\n"
		"        telnet %s\n\n\n", MY_BBS_DOMAIN);
	fprintf(fout, "                              %s BBS\n", MY_BBS_NAME);
	fclose(fout);
	retv = bbs_sendmail((char*)tmpfn, title, (char*)toemail, (char*)inviter, 0);
	if (retv < 0)
		return -4;
	return 0;
}
