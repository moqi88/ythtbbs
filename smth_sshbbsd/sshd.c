...
int main(int ac, char *av[])
{
    ...
    if (!done) {
        if (debug_flag) {
            ...
            log_init(av0, !inetd_flag, 1, 1, options.log_facility);
...
void do_authentication(char *user, int privileged_port, int cipher_type)
{
    ...
    if (*user == '\0' || !(usernum = getuser(user, &currentuser)) || userbansite(user,get_remote_ipaddr()))
	do_authentication_fail_loop();
	...
	if (currentuser->salt == 0) {
		...
		genpasswd(md5pass, tmpu.salt, "");
		memcpy(tmpu.passwd, md5pass, 16);
		substitute_record(PASSFILE, &tmpu, sizeof (struct userec), usernum);
	}
    ...
    while (!authenticated) {
        ...
        case SSH_CMSG_AUTH_PASSWORD:
	        ...
	        if (currentuser->salt == 0) {
                    memcpy(&tmpu, currentuser, sizeof(struct userec));
                    tmpu.salt = getsalt_md5();
                    genpasswd(md5pass, tmpu.salt, password);
                    memcpy(tmpu.passwd, md5pass, 16);
                    substitute_record(PASSFILE, &tmpu, sizeof (struct userec), usernum);
                }
