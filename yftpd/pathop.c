...
search_board(char *board)
{
	...
	struct BCACHE *shm_bcache = bbsinfo.bcacheshm;
...
my_quota_init()
{
	...
	if (quotaf->hasinit != 1) {
		...
		quotaf->q[0] = (long long)600 * 1024 * 1024;
		strcpy(quotaf->d[0], "ÏµÍ³Ê£Óà´ÅÅÌÅä¶î");
		quotaf->q[1] = (long long)600 * 1024 * 1024;
		strcpy(quotaf->d[1], "°æÃæÊ£Óà´ÅÅÌÅä¶î");
		quotaf->hasinit = 1;
	}
	quotaf->changed = 0;
	i = shmget(getBBSKey(YFTP_SHM), sizeof (*quota), IPC_CREAT | S_IRWXU);
	if (i < 0)
		return -1;
