...
print_file(int number, char *filename)
{
	...
	if (file) {
		while (fgets(foo, sizeof (foo), file)) {
			ptr = strtok(foo, "\r\n");
			if (!ptr)
				prints("%i-", number);
		}
...
do_sem()
{
	...
	struct UTMPFILE *shm_utmp = bbsinfo.utmpshm;
...
main(int argc, char *argv[])
{
	...
	flow_semid = semget(getBBSKey(YFTP_SEM), 1, IPC_CREAT | S_IRWXU);
	if (flow_semid < 0)
		exit(0);
	if (my_quota_init() < 0)
		exit(0);
