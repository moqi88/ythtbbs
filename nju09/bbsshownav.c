...
shownavpart(int mode, const char *secstr)
{
	...
	if (file_time("wwwtmp/navpart.txt") >= loadtime) {
		...
		if (fp == NULL)
			return -1;
		count = 0;
		while (count < 120 && fgets(buf, sizeof (buf), fp) != NULL) {
...
shownavpartline(char *buf, int mode)
{
	...
	*(title++) = 0;
	...
	if (mode == 0) {
		printf("<tr><td><div style='overflow: hidden; "
				"text-overflow: ellipsis; white-space: "
				"nowrap;'><li /><a href='bbsnt?B=%d&th=%s'>",
		       getbnumx(x1), ptr);
		...
		printf("</font></div></td></tr>");
	} else {
		printf
		    ("<tr><td>%d</td><td>%s</td><td><a href='home?B=%d'>%s</a></td>"
		     "<td>", star, numstr, getbnumx(x1), boardstr);
		printf("%s</td><td><a href='bbsnt?B=%d&th=%d'>%s</a>"
