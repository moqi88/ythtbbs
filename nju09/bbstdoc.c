...
bbstdoc_main()
{
	...
	MMAP_TRY {
		...
		if (start == 0) {
			...
			num = Search_Bin((struct fileheader*)mf.ptr, start, 0, total - 1);
			if (1 == direction) {
				if (num < 0)
					num = -(num + 1);
				else
		...
		printf(" <a href=bbspst?B=%d class=red>我要发表文章!</a> ", getbnumx(x1));
	...
	if (!hasmore) {
		...
		for (i = 0; boardaux && i < boardaux->ntopfile; i++) {
			...
			title[60] = 0;
			printf
			    ("<td colspan=2><a href=bbstopcon?B=%d&F=%s>%s%s</a>%s</td></tr>",
	...
	printhr();
	docinfostr(x1, 1, has_BM_perm(currentuser, x1));
	//printf(" <a href=tshirt><font color=red>订购糊涂站衫</font></a> ");
	printf("<a href=bbspst?B=%d><font color=red>我要发表文章!</a></a> ", getbnumx(x1));
	if (!(nothingmore && direction == -1))
		printf("<a href=tdoc?B=%d&S=%d&D=-1>上一页</a> ", getbnumx(x1),
		       (direction == 1) ? first : last);
	if (!(nothingmore && direction == 1))
