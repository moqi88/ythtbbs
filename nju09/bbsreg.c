...
//      printf("function show(userid) {if (document.images) document.images['myImage'].src = \"http://"MY_BBS_DOMAIN"/cgi-bin/regimage?userid=\"+userid;}\n");
	...
	printf
	    ("<tr><td align=right>*请输入代号:<td align=left><input name=userid size=%d maxlength=%d onBlur=show(document.regform.userid.value);return false> (3-12字符, 必须全为英文字母或标准汉字)\n",
	     IDLEN, IDLEN);
	printf
	    ("<tr><td align=right>*请输入密码:<td align=left><input type=password name=pass1 size=%d maxlength=%d> (有效长度31位)\n",
	     PASSLEN - 1, PASSLEN - 1);
	printf
	    ("<tr><td align=right>*请确认密码:<td align=left><input type=password name=pass2 size=%d maxlength=%d> (有效长度31位)\n",
	     PASSLEN - 1, PASSLEN - 1);
	printf
	    ("<tr><td align=right>*请输入昵称:<td align=left><input name=username size=20 maxlength=%d> (2-30字符, 中英文不限)\n",
	     NAMELEN - 1);
	printf
	    ("<tr><td align=right>*请输入您的真实姓名:<td align=left><input name=realname size=20> (请用中文, 至少2个汉字)\n");
	printf
	    ("<tr><td align=right>*目前详细住址:<td align=left><input name=address size=40>  (至少8个汉字)\n");
	//printf
	 //   ("<tr><td></td><td align=left><font color=red>(住址请具体到宿舍或者门牌号码)</font></td></tr>");
	printf
	    ("<tr><td align=right>*学校系级或者公司单位:<td align=left><input name=dept size=40> (至少7个汉字)\n");
	//printf
	 //   ("<tr><td></td><td align=left><font color=red>(学校请具体到院系年级，工作单位请具体到部门)</font></td></tr>");
	printf
	...
	printf
	    ("<tr><td align=right><input type=\"button\" name=g value=\"请先填写代号\" disabled=true onClick=show(document.regform.userid.value);return false><br>");
	printf
	    ("<td align=left><img src=\"/cgi-bin/regimage\" name=\"myImage\">");
	printf
	    ("<tr><td align=right>*请输入上面的数字:<td align=left><input name=regpass size=10 maxlength=10>\n");
	printf("</table><br><hr>\n");
	printf
