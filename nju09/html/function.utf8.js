...
function adjustMypic(pic) {
	if(pic.width>120||pic.height>160) {
		if(pic.width/pic.height>0.75)
			pic.width=120;
		else 
			pic.height=160;
	}
...
function
printMypic(magic, user, custom)
{
	var img;
	if(custom)
		img='/'+magic+'/mypic?U='+user;
	else
		img='/defaultmypic.gif';
	document.writeln('<a href=qry?U='+user+'><center>' +
		'<img src='+img+' alt="'+user+'的头像（在登录后左侧菜单中的个人工具箱内，可以增添或修改您的头像）" border=0 onLoad="javascript: adjustMypic(this);">'
		+'</center></a>');
}
