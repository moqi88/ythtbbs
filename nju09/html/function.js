...
function docItem(flag, noreply, author, fname, edt, title, size, star, neval, manager, bnum, start)
{
	...
	if(manager) {
		...
		document.write("fname="+fname+">锁</a> ");
...
function lowerTag(str)
{
	...
	function url(s, s1, s2) {
		...
		newstr +='<font color=green>外部图片: </font>'; 
		newstr += url + "<br></div>";
		return newstr;
	}
	pattern1 = /<([^><]*)(\.width>)/i;
...
lm2str(bl)
{
	...
	if(bl.isclose==1)
		return '<div class=gry>封闭版面</div>';
	if(bl.isclose==2)
		retv+='<div class=gry>只读版面</div>';
...
oneBoard(bl)
{
	...
	retv+="<div class\"clear\"></div>\
		...
		<img border=0 alt=\"版面图标\" src=\""+icon+"\"></a>\
		</div>"+a+"\
		<li>人　气："+bl.score+"</li>\
		<li>文章数："+bl.total+"</li>\
		<li>版　主："+bl.bm+"</li>\
...
function showdocnav(mode, bnum, hastmpl, hasbmperm, start, pagesize, total) {
	...
	document.write("<td>[ ");
	if (mode == 0)
		document.write("一般模式 <a href=tdoc?B="+bnum+">主题模式</a> ");
	else
		document.write("<a href=doc?B="+bnum+">一般模式</a> 主题模式 ");
	if (hasbmperm)
		document.write("<a href=# onclick=switch_manop()> 管理模式</a> ");
	document.write("]</td>");
	//document.write("<td><a href=home?B=Tshirt class=red>糊涂站衫</a></td>");
	...
	document.write("<form name=docform2 method=post action="+(mode==0?'doc':'tdoc')+"> \
		...
		value='讨论区' onfocus=\"this.value=''\"></td></form>");
	document.write("<td><a href=# onclick=doc_go_begin("+mode+","+bnum+","+hasbmperm+")>首页</a> ");
	if (mode == 0 && start <= 1)
		document.write("上一页 ");
	else
		document.write("<a href=# onclick=\
			doc_go_prev("+mode+","+bnum+","+start+","+pagesize+","+hasbmperm+")>上一页</a> ");
	if (mode == 0 && start < total - pagesize + 1)
		document.write("<a href=# onclick=\
			doc_go_next("+mode+","+bnum+","+start+","+pagesize+","+total+","+hasbmperm+")>下一页</a> ");
	else
		document.write("下一页 ");
	document.write("<a href=# onclick=doc_go_end("+mode+","+bnum+","+hasbmperm+")>末页</a></td>");
...
function checkFrame(redirect)
{
	...
	// 注释掉 for SEO 流量   Do not know why use this function....

	if (!redirect)
	//修改了框架，去掉了显示完整界面
//		document.writeln("<a href=bbsindex?t=1&b="+d.substring(temp.lastIndexOf('/')+1, d.length)+">显示完整界面</a> ");
		;
	else
...
function bbspst_insertBokeccUrl(url) {
	...
	url += '这里有一个视频，请用WEB方式观看。</font></div>';
