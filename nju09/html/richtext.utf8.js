...
function writeRTE(rte, html, width, height, buttons, readOnly) {
	...
	if(height=="100%")
		...
		document.writeln('<table width="'+width+'" cellpadding="0" cellspacing="0" style="table-layout:fixed"><tr height=70><td>');
	...
	if (buttons == true) {
		...
		if (!readOnly)
			document.writeln('<input type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\');" />源码');
