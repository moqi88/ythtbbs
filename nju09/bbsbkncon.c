...
bbsbkncon_main()
{
	...
	MMAP_TRY {
		...
		showcon(filename);
		printf("[<a href=bbsfwd?B=%d&file=%s>推荐给朋友</a>]", getbnumx(brd),
		       file);
		printf("[<a href=bbsccc?B=%d&file=%s>转贴</a>]", getbnumx(brd),
		       file);
		if (num > 0) {
		...
		printf("[<a href=bbsbkndoc?B=%d&bkn=%s>本卷过刊</a>]",
		       getbnumx(brd), bkn);
		if (num < total - 1) {
			x = (struct fileheader *) (mf.ptr +
						   (num +
						   ...
						   sizeof (struct fileheader));
			printf
			    ("[<a href=bbsbkncon?B=%d&bkn=%s&file=%s&num=%d>下一篇</a>]",
			     getbnumx(brd), bkn, fh2fname(x), num + 1);
		}
	}
	MMAP_CATCH {
