...
struct cgi_applet applets[] = {
	...
	{index2009_main, {"index2009", "2009", NULL}},
	...
	{bbstopic_main, {"bbstopic", "topic", NULL}},
...
//souke_main is commented, no use now
//	{bbssouke_main, {"bbssouke", NULL}},
	...
	{bbspsttmpl_main, {"psttmpl", NULL}},
	{bbspassport_main, {"passport", NULL}},
	{bbslpassport_main, {"lpassport", NULL}},
	{bbsshowdigest_main, {"digest", NULL}},
	{bbstshirt_main, {"tshirt", NULL}},
	{bbsspecial_main, {"special", NULL}},
#ifdef ENABLE_BLOG
	...
	{blogeditlink_main, {"blogeditlink", NULL}},
	{blogeditfriend_main, {"blogeditfriend", NULL}},
	...
	{blogmedia_main, {"blogmedia", NULL}},
	{blogdelete_main, {"blogdelete", NULL}},
	{blogtemplate_main, {"blogtemplate", NULL}},
	{blogblock_main, {"blogblock", NULL}},
	{blogpic_main, {"blogpic", NULL}},
#endif
	{bbsseo_main, {"seo", "s", NULL}},
	{bbsajaxtest_main, {"ajax", NULL}},
	{bbsadv_main, {"bbsadv", NULL}},
	{bbsfootball_main, {"bbsfb", "bbsfootball", NULL}},
	{bbsbug_main, {"bbsbug", "bug", NULL}},
	{bbstestssl_main, {"tssl", NULL}},
	{bbsregcheck_main,{"regcheck",NULL}},
	{NULL}
};
...
main(int argc, char *argv[])
{
	...
	get_att_server();
	get_tjpg_server();
	c_ctx = cuser_init("127.0.0.1");
	key_fail=load_all_key();
	while (FCGI_Accept() >= 0) {
#ifdef USEBIG5
		start_outcache();
#endif
		...
		now_t = time(NULL);
		via_proxy = 0;
		strsncpy(realfromhost, getsenv("REMOTE_ADDR"), sizeof (realfromhost));
		...
		if (via_proxy
			...
			if (p != NULL) {
				while (!isdigit(*p) && *p)
					p++;
				if (*p)
					strncpy(realfromhost, p, sizeof(realfromhost));
				else
					strncpy(realfromhost, ptr, sizeof(realfromhost));
			} else
				strncpy(realfromhost, ptr, sizeof(realfromhost));
			inet_aton(realfromhost, &from_addr);
                        in = from_addr;
