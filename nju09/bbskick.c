...
printkickitem(const struct user_info *x)
{
	...
	printf
	    ("<a onclick='return confirm(\"你真的要踢出该用户吗?\")' href=kick?k=%s&f=%s>X</a></td>",
	     x->userid, x->from);
...
printkicklist(const char *usertokick)
{
	...
	printf
	    ("<tr><td>序号</td><td>使用者代号</td><td>使用者昵称</td><td>动态</td><td>发呆</td></tr>\n");
...
bbskick_main()
{
	...
	changemode(LUSERS);
	tokick = getparm("k");
	if (strlen(tokick) == 0) {	//没有用户名参数k就要求输入一个,但不检查f
		printf
		    ("<hr><br><center><form action=kick name=userkicker method=post>");
		printf
		    ("请输入希望查询其在线情况的用户id  <input type=text name=k ><input type=submit value=查看></form><hr>");
	} else {
