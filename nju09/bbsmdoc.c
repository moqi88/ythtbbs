...
bbsmdoc_main()
{
	...
	for (i = 0; i < 20; i++) {
		...
		printf
		    ("<td><input style='height:18px' name=box%s type=checkbox>",
		     fh2fname(&x));
		printf("<td>%s<td>%s", flag_str_bm(x.accessed, 1),
		       userid_str(fh2owner(&x)));
		printf("<td>%12.12s", Ctime(x.filetime) + 4);
		x.title[40] = 0;
		printf("<td><a href=con_%d_%s.htm?N=%d&T=%d>%s%s </a>\n",
	...
	printf("</form>\n");
	if (start > 0) {
		printf("<a href=bbsmdoc?B=%d&start=%d>上一页</a> ",
		       getbnumx(x1), start < 20 ? 0 : start - 20);
	}
	if (start < total - 20) {
		printf("<a href=bbsmdoc?B=%d&start=%d>下一页</a> ",
		       getbnumx(x1), start + 20);
	}
	printf("<a href=bbsdoc?B=%d>一般模式</a> ", getbnumx(x1));
	printf("<a href=bbsdenyall?B=%d>封人名单</a> ", getbnumx(x1));
	printf("<a href=bbsmnote?B=%d>编辑备忘录</a> ", getbnumx(x1));
