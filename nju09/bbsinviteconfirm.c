...
bbsinviteconfirm_main()
{
	...
	if (!file_exist(invitefn) ||
		...
		printf
		    ("这个邀请已经过期了，但是您可以进行<a href=bbsemailreg>自动注册</a>。");
		return 0;
	}

	if (!trustEmail(email)) {
		printf("所使用的 email 已经注册了别的用户，或者不在认证范围。");
		return 0;
	}
	printf
	...
	printf("<input type=hidden name=name value='%s'>", encode_url(name));
	printf("<table>\n");
