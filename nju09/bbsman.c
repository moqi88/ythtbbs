...
int bbsman_main() {
	int bnum, size, fd, start, direction, mode, operation;
	struct boardmem *x;
	char filename[80], board[80];
	char dir[80], *data = NULL;
	...
	mode = atoi(getparm("M"));
	operation = atoi(getparm("O"));
	strncpy(filename, getparm("F"), sizeof(filename));
	bnum = getbnumx(x);
	MMAP_TRY {
		data =
		...
		if (operation == 1) {
			...
			do_set(data, size, filename, FH_DEL, board);
#else
			do_del(board, filename);
#endif
		} else if (operation == 2)
			do_set(data, size, filename, FH_MARKED, board);
		else if (operation == 3)
			do_set(data, size, filename, FH_DIGEST, board);
		else if (operation == 4)
			do_set(data, size, filename, FH_NOREPLY, board);
		else if (operation == 5)
			do_set(data, size, filename, 0, board);
	}
	MMAP_CATCH {
		close(fd);
	}
	...
	printf("<script>location.replace('doc?B=%d&M=%d&S=%d&D=%d&manop=1');</script>", 
			bnum, mode, start, direction);
	http_quit();
	return 0;
...
bbsman_main()
{
	...
	printf("<br><a href=bbsmdoc?B=%d>返回管理模式</a>", getbnumx(brd));
	http_quit();
	return 0;
}*/
