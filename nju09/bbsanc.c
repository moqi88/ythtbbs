...
static int bbsanc_show_article_link(char *path, char *item);
...
//修改框架，增加底部
	...
	showcon(fn);
	// show article link
	bbsanc_show_article_link(path, item);
	...
	fclose(fp);
	if (board[0])
		printf("[<a href=doc?B=%d>本讨论区</a>]", getbnum(board));
...
static int
bbsanc_show_article_link(char *path, char *item)
{
	char link[PATH_MAX];
#ifndef USEBIG5
	snprintf(link, sizeof(link), "http://%s/" BASESMAGIC
		"/bbsanc?path=%s&item=%s", getsenv("HTTP_HOST"), path, item);
#else
	snprintf(link, sizeof(link), "http://%s/" BASESMAGICBI5
		"/bbsanc?path=%s&item=%s", getsenv("HTTP_HOST"), path, item);
#endif
	printf("全文链接: <a href=%s target=_blank>%s</a><br>", link, link);
	return 0;
}
