...
struct wwwstyle wwwstyle[NWWWSTYLE] = {
	...
	{"一见如故新体验(小字体)", CSSPATH "bbssf.css", CSSPATH "leftsf.css"},
	{"一见如故新体验(大字体)", CSSPATH "bbslf.css", CSSPATH "leftlf.css"},
	{"自定义的界面", "bbsucss/ubbs.css", "bbsucss/uleft.css"}
};
struct wwwstyle *currstyle = &(wwwstyle[DEFAULT_WWWSTYLE]);
int wwwstylenum = DEFAULT_WWWSTYLE;
int contentbg = 0;
...
void
strnncpy(char *s, int *l, const char *s2)
{
	int l2 = strlen(s2);
	memcpy(s + (*l), s2, l2);
	(*l) += l2;
...
#ifdef ENABLE_BLOG
static char *blogdomainname[] = {
	"blog." MY_BBS_DOMAIN, 
	"blog.yjrg.com", 
	"blog.yjrg.org", 
	"blog.proxy." MY_BBS_DOMAIN,
	"blog.proxy.yjrg.com",
	"blog.proxy.yjrg.org",
	"b." MY_BBS_DOMAIN, 
	"b.yjrg.com", 
	"b.yjrg.org", 
	"b.proxy." MY_BBS_DOMAIN,
	"b.proxy.yjrg.com",
	"b.proxy.yjrg.org",
	NULL
};
...
static char *pcdomainname[] = {
	"p." MY_BBS_DOMAIN, 
	"p.yjrg.com", 
	"p.yjrg.org",
	"p.proxy." MY_BBS_DOMAIN,
	"p.proxy.yjrg.com",
	"p.proxy.yjrg.org",
	NULL
};
...
//未能进入登录统计，只记录session的guest，其参量
//def_mode是版面模式，0为默认一般模式。
...
{
	/*
	The difference between SCRIPT_URL and SCRIPT_URI:
	SCRIPT_URL	/HT/doc?B=18
	REQUEST_URI	http://yjrg.net/HT/doc?B=18
	*/
	char *url, *end, name[STRLEN], *p, *extraparam;
	url = getenv("SCRIPT_URL");
	if (NULL == url) {
		//If SCRIPT_URL is NULL, get it from REQUEST_URI
		char buf[1024];
	...
	if (strcmp(url, "/")) {
		...
		//tracelog("SESSION is %s", getparm("SESSION"));
	}
...
#else
	if (!strncmp(url, "/" SMAGIC, sizeof (SMAGIC))) {
		//when requesting a cgi program
		snprintf(name, STRLEN, "%s", url + sizeof (SMAGIC));
		p = strchr(name, '/');	//get the cgi name
		if (NULL != p) {
			*p = 0;
			url = strchr(url + 1, '/');
					//get the whole url without SMAGIC
		} else
			return -1;
	}
#endif
	extraparam = strchr(name, '_');	//get css param
	if (extraparam) {
		*extraparam = 0;
		extraparam++;
	} else
	...
	if (!*needcgi || nologin) {	
				//set the cgi to be the default index page.
		strcpy(needcgi, "bbsindex");
		strcpy(rframe, "");
		strsncpy(name, getsenv("HTTP_HOST"), 70);
		p = strchr(name, '.');
		if (p != NULL) {
			if (isaword(domainname, p + 1)) {
				*p = 0;
				strsncpy(rframe, name, 60);
#ifdef ENABLE_BLOG
			} else if (isaword(blogdomainname, p + 1)) {
				*p = 0;
				snprintf(rframe, 60, "%s.blog", name);
#endif
			} else if (isaword(pcdomainname, p + 1)) {
				*p = 0;
				snprintf(rframe, 60, "%s.p", name);
			}
		}
...
http_parm_init(void)
{
	...
	if (n > 5 * 1024 * 1024)
		http_fatal("File size must be no more than 5M bytes.");
	buf = malloc(n + 1);
	if (NULL == buf)
		http_fatal("Out of memory.");
...
html_header(int mode)
{
	...
	case 11:		//bbsgetmsg
		...
		printf("<style type=text/css>"
		       ...
		       "</style>");
#endif
		printf("<script src=\"" BBSJS "?%d\" type=\"text/javascript\"></script>\n", cssv);
		break;
	case 2:
		printf
		    ("<head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=%s\">",
...
extraparam_init(unsigned char *extrastr)
{
	...
	if (*extrastr) {
		...
		currstyle = &wwwstyle[DEFAULT_WWWSTYLE];
}
	  ...
	  char *nickname, char *ip, int sig, int mark)
{
	...
	sprintf(buf, "4 %s %s", currentuser->userid, userid);
	friendslog(buf);		      
	return 0;
}
		  ...
		  int outgoing, char *realauthor, int thread)
{
...
#ifdef ENABLE_BLOG
	int blog = (thread == -2);
	char bloglink[STRLEN];
#endif
	struct tm *n;
	n = localtime(&now_t);
#ifdef ENABLE_BLOG
	if (blog) {
		strncpy(bloglink, id, sizeof(bloglink));
		id = realauthor;
	} else
		bloglink[0] = 0;
#endif
	...
	fprintf(fp, "\n--\n");
	sig_append(fp, id, sig);
	fprintf(fp, "\033[m\033[1;%dm※ 来源:．%s %s [FROM: %.20s]",
		31 + rand() % 7, BBSNAME, "http://" MY_BBS_DOMAIN, ip);
#ifdef ENABLE_BLOG
	if (blog)
		fprintf(fp, "\n※ BLOG:．http://%s/%s/%s", 
			MY_BBS_DOMAIN, SMAGIC, bloglink);
#endif
	fprintf(fp, "\033[m\n");
	fclose(fp);
...
{
	FILE *fp, *fp2;
	char buf3[1024], buf[STRLEN], buf2[STRLEN];
	struct fileheader header;
	int t, i;
#ifdef ENABLE_BLOG
	int blog = (thread == -2);
	...
	if (!hideboard(board)) {
		...
		sethomefile(buf, id, "wwwnewpost");
		sethomefile(buf2, id, "wwwnewpost.tmp");
		if (NULL == (fp2 = fopen(buf2, "w+")))
			return t;
		fprintf(fp2, "%d\t%s\t%s\n", t, board, title);
...
int https_only_boards(struct boardmem *x) {
	if (!(x->header.flag & CLOSECLUB_FLAG))
		return 0;
	if (strcasecmp(getsenv("HTTPS"), "ON") && 
			seek_in_file("etc/ssh_only_boards", 
				x->header.filename))
		return 1;
	return 0;
}
...
getboard(char board[80])
{
	...
	if (!has_read_perm_x(currentuser, x1))
		return NULL;
	if (https_only_boards(x1))
		http_fatal("目前本版只能通过HTTPS或SSH方式访问。");
...
changemode(int mode)
{
	if (!loginok || u_info->mode == mode)
		return;
	u_info->mode = mode;
	if (mode != BACKNUMBER && mode != READING && mode != POSTING && mode != POSTTMPL)
		updatelastboard();
	return;
}
...
//如果你要用这个的话，有可能其实你要用的是encode_url。
char *
...
char *
flag_str_bm(int access, int has_read)
{
	static char buf[80];
	strcpy(buf, "    ");
	if (access & FH_DIGEST)
...
// mode = 1 表示 .DIR 按时间排序  0 表示 不排序
{
	...
	if (*num < 0) {
		*num =
		    Search_Bin((struct fileheader *) mf->ptr, filetime, 0,
			       total - 1);
		if (*num >= 0) {
			ptr =
...
char myfolder[FOLDER_NUM][80];
char mybrd[FOLDER_NUM][FAVOR_BRD_NUM][80];
//int mybrdnum = 0;
int totalmybrdnum=0;
int mybrdnum[FOLDER_NUM];
int myfoldernum=0;
...
printhr()
{
	...
	//printf("<script>phr();</script>");
#if 0
	printf("<TABLE width=100%% cellspacing=1 cellpadding=0>"
	       "<TR class=hrcolor><TD><IMG height=1 src=\"/small.gif\""
	       " width=1></TD></TR></table>");
#endif
}
...
updatewwwindex(struct boardmem *x)
{
	...
	sprintf(buf, "ftphome/root/boards/%s/link/config.ini", x->header.filename);
	x->wwwlink = access(buf, R_OK) ? 0 : 1;
	size = file_size(buf);
	if (size > 10 && size < 12 * 1024)
		x->wwwlink = 1;
	else
...
void print_caution_box (int mode, char * msg) {
	printf("<div align=center>"
			"<div style='text-align: left; border-style: dotted;"
			"border-width: 1px; width: 450px; padding: 10px;"
			"background-image: url(%s); background-repeat: no-repeat;"
			"background-position: center center;'>"
			"<p style='border-style: solid; border-width: 0 0 1 0px; "
			"border-color: #67a8ea; text-align: center; color: %s'>"
			"%s</p>"
			"<div style='padding: 5px; height: 120px; overflow: auto; "
			"margin: 15 20 15 20px; border-color: #888; border-style: solid; "
			"border-width: 1px;'>%s</div>",
			mode == 0 ? "/images/stop.gif" : "/images/caution.gif", 
			mode == 0 ? "red" : "blue", 
			mode == 0 ? "系统严重错误" : "系统提示", 
			msg);
	printf("<div align=center><a href=\"home?B=7\">去BBSHelp版提问</a>　"
			"<a href=\"psttmpl?B=0&action=post&num=2\">向站长报告Bug（请先登陆）</a>　"
			"<a href=\"javascript: history.back();\">后退一步</a></div></div></div>");
	return;
}


int isSpider() {
	const char *spiderList[20] = {
		"Baiduspider", 
		"Googlebot", 
	...
	char *agent = getsenv("HTTP_USER_AGENT");
	int i = 0;

	while (spiderList[i] != NULL) {
		if (strcasestr(agent, spiderList[i]))
			return 1;
		i++;
	}
	return 0;
