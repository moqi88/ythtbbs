...
bbstfind_main()
{
	...
	MMAP_TRY {
		...
		for (; i < numrecords; i++) {
			...
			if (total == 0) {
				printf
				    ("<a href=doc?B=%d&start=%d>本讨论区</a> ", getbnumx(x1), (i > 4) ? (i - 4) : 1);
				printf
				    ("<a href=tcon_%d_%d.htm?start=%d>本主题全部展开</a> ",
				     getbnumx(x1), thread, i);
				printf
			...
			printf
			    ("<td><a href=con_%d_%s.htm?N=%d&st=1&T=%d>%s </a>\n",
	...
	printf("</table><hr>\n共找到 %d 篇 \n", total);
	printf
	    ("<a href=doc?B=%d&start=%d>本讨论区</a> ",
	     getbnumx(x1), (start > 5) ? (start - 5) : 1);
	if (total > 0) {
		printf
		    ("<a href=tcon_%d_%d.htm?start=%d>本主题全部展开</a> ",
		     getbnumx(x1), thread, start - 1);
