...
struct userec *a, *b;
{
	return (a->numlogins  + a->numposts/5 + a->stay / 1800 ) -
	    (b->numlogins + b->numposts/5 + b->stay / 1800 );
}
...
top_perm(FILE * fp)
{
	...
	fprintf(fp, "\n\n\n%s", "\
              \033[1;37m===========    \033[36m总表现积分排行榜\033[37m    ============ \n\
                  \033[32m 公式：上站次数+文章数/5+上站总时数*2\033[37m\n\
...
");
	for (i = 0; i < rows; i++) {
		sprintf(buf1, "[%2d] %-10.10s %-14.14s %5ld",
			...
			(perm_q[i].numlogins ) + perm_q[i].numposts/5 +
			(perm_q[i].stay / 1800));
		j = i + rows;
		sprintf(buf2, "[%2d] %-10.10s %-14.14s   %5ld",
			j + 1, perm_q[j].userid, perm_q[j].username,
			(perm_q[j].numlogins ) + perm_q[j].numposts/5 +
			(perm_q[j].stay / 1800) );
...
output_c(char *file, int mode)
{
	...
	sprintf(fn, MY_BBS_HOME "/%s.new", file);
	sprintf(oldfn, MY_BBS_HOME "/%s", file);
	fd = open(fn, O_TRUNC| O_WRONLY | O_CREAT, 0660);
	if (fd) {
		qsort(ms, count, sizeof(struct mystat), (void *)cmpstat);
		for (i=1; i<count;i++) {
			(ms+i)->num += (ms+i-1)->num;
		}
