...
{
	FILE *fp;
	char filen[800], str[500],webn[800],webpre[800];
	char title[100],*ptr;
	struct stat sbuf;
	int i, n, hide = 0;
	time_t t;
	...
	while (fgets(str, 500, fp) != NULL) {
		...
		sprintf(webn,"%.300s",str+strlen("Path=~/"));
		sprintf(webpre,"%.300s",path+strlen("/home/bbs/0Announce"));
		//printf("filen: %s\n", filen);
		...
		if (S_ISDIR(sbuf.st_mode)) {
			...
			if (indextype == 3) {
				if (thisnew) {
					...
					ncachetitle = 0;
					fprintf(wfp,
						"\033[<a href=bbs0an?path=%s&amp;/%s target=_blank>%s%3d.[\033[33m目录\033[m]%s\033[</a>\n",
						webpre,webn,prefix, n, title);
				} else
					sprintf(cachetitles[ncachetitle++],
						"\033[<a href=bbs0an?path=%s&amp;/%s target=_blank>%s%3d.[\033[33m目录\033[m]%s\033[</a>\n",
						webpre,webn,prefix, n, title);
			} else if (indextype == 2 && thisnew)
				fprintf(wfp, "\033[<a href=bbs0an?path=%s/%s target=_blank>☆%s%3d.[\033[33m目录\033[m]%s\033[</a>\n",
					webpre,webn,prefix, n, title);
			else
				fprintf(wfp, "　\033[<a href=bbs0an?path=%s/%s target=_blank>%s%3d.[\033[33m目录\033[m]%s\033[</a>\n",
					webpre,webn,prefix, n, title);
			...
			if (indextype == 3) {
				if (thisnew) {
					...
					ncachetitle = 0;
					fprintf(wfp,
						"　\033[<a href=bbsanc?path=%s&amp;item=/%s target=_blank>%s%3d.[\033[33m文件\033[m]%s\033[</a>\n",
						webpre,webn,prefix, n, title);
				}
			} else if (indextype == 1)
				fprintf(wfp, "\033[<a href=bbsanc?path=%s&amp;item=/%s target=_blank>%s%s%3d.[\033[33m文件\033[m]%s\033[</a>\n",
					webpre,webn,thisnew ? "☆" : "　", prefix, n,
					title);
		} else
			continue;
	}
