...
int water_article(char *bname, struct fileheader *fh) {
	...
	if(fd < 0)
		return 1;
	fstat(fd, &st);
	if(st.st_size < (200 + 93)) //天头地脚大约200字
		retv =  1;
	close(fd);
	return retv;
}
...
html_topten(int filter_mode, int sec)
{
	...
	if(sec == 0)
		...
		fprintf(fp, "<center class=f2>今日%c区十大热门话题\n\n", sec);
		
	fprintf(fp, "<table border=1>\n");
	fprintf
	    (fp,
	     "<tr><td>名次</td><td>讨论区</td><td>标题</td><td>人数</td><td>作者</td></tr>\n");
...
telnet_topten(int mode, char *file)
{
	...
	for (j = 0; j < 10 && bt->unum != 0; j++, bt++) {
		...
		p = buf + 4;
		fprintf(fp,
			"\033[1;37m第\033[1;31m%3d\033[37m 名 \033[37m信区 : \033[33m%-16s\033[37m【\033[32m%s\033[37m】\033[36m%4d\033[37m 人  \033[35m%12s\033[m\n     \033[37m标题 : \033[1;44;37m%-60.60s\033[40m\n",
