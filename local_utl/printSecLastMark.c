...
makeboardlist(const struct sectree *sec, struct boardlist *bl, int max)
{
	...
	snprintf(admin2, sizeof (admin2), "%sadmin", sec->basestr);
...
printseclastmark(FILE * fp, const struct sectree *sec, int nline)
{
	...
	fprintf(fp, "sec_mark_start('%s', '%s');\n", sec->basestr, scriptstr(sec->title));
	...
	if (pcount < count) {
		if (sec->parent != &sectree)
			...
			l = 3;
		fprintf(fp, "sec_mark_boardlist('%s',new Array(", sec->basestr);
	...
	/*if (!count)
		fprintf(fp,
			"document.write(\"本分区目前没有被标记的文章。\");");*/
	fprintf(fp, "sec_mark_end();\n\n");
	return n;
}
...
makeallseclastmark(const struct sectree *sec)
{
	...
	if (!fp)
		return -1;
	fprintf(fp, "<!--\n"
		"document.write(\"<div id=\\\"mark\\\"><div id=\\\"mark_left\\\">\");\n");
	...
	for (i = 0; i < n; i++) {
		if (i == separate)
			fprintf(fp,
				"document.write(\"</div><div id=\\\"mark_right\\\">\");\n");
		if (sec->introstr[i] == '|')
			continue;
	...
	fprintf(fp, "document.write(\"</div></div>\");\n//-->\n");
	fclose(fp);
...
printlastmarkline(FILE * fp, struct lastmarkl *lm, int i)
{
	...
	fprintf(fp, "sec_mark_item('%d','%s',%d,", lm->bnum, scriptstr(lm->boardtitle),
		lm->thread[i]);
