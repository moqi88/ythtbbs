...
#define BMINVACATION MY_BBS_HOME"/0Announce/groups/GROUP_0/BMClub/vacation"
...
int bm_inVacation (char *userid) {
	char buf[256], *ptr;
	FILE *fp;

	if ((fp = fopen(BMINVACATION, "r")) == NULL)
		return 0;
...
void bm_vacationClear() {
	...
	if ((fp = fopen(BMINVACATION, "r")) == NULL)
		return;
	if ((newfp = fopen(BMINVACATION ".new", "r")) == NULL) {
		fclose(fp);
		return;
	}
	while (fgets(buf, sizeof(buf), fp)) {
	...
	fclose(newfp);
	unlink(BMINVACATION);
	rename(BMINVACATION ".new", BMINVACATION);
}
...
load_bmstat_history_file()
{
	...
	fread(bmstat_history_mem_rd, sizeof (struct board_bmstat), MAXBOARD, fp);
	fclose(fp);
	return 0;
}
...
save_bmstat_history_file()
{
	...
	fwrite(bmstat_history_mem_wr, sizeof (struct board_bmstat), MAXBOARD, fp);
	fclose(fp);
...
send_warning_mail(char *boardname, struct bm_eva *bmeva)
{
	...
	snprintf(content, 255, "%s版版主%s:\n"
		 ...
		 "    具体情况可到相应区务管理版或BMClub版查看\n"
		 "    望今后能加强版务工作, 一见如故BBS向您的辛勤工作表示感谢!\n\n",
		 boardname, bmeva->userid, bmeva->week);
	sprintf(title, "%s", "[系统通知]抱歉, 您的版务考勤不合格");
	system_mail_buf(content, strlen(content), bmeva->userid, title, "deliver");
}
...
static void
gen_report_header(char currSec, FILE *fp_p, FILE *fp_p2, char *POST_NAME, char *POST_PASS, char *fn)
{
	...
	for (i = 0; i < bmc; i++) {
		...
		if (data->noread) {
			...
			if (currSec != data->sec) {
				...
				if (nrule > 0 && !onvac)
					gen_report_header(currSec, fp_p, fp_p2, POST_NAME, POST_PASS, fn);
			}
		}
		fprintf(fp,
		...
		if ((nrule > 0) && (!data->noread) && !onvac) {
			...
			if (bm_inVacation(data->userid))
				fprintf(fp_p, "\033[0;32m");
			fprintf(fp_p, "%-15s%-20s", data->userid, data->board);
			if (fp_p2) {
				if (bm_inVacation(data->userid))
					fprintf(fp_p2, "\033[0;32m");
			...
			for (j = 0; j <= ngrp; j++) {
				...
				for (m = 5; m < k; m++)
					result[m] = ' ';
				result[m] = '\0';
				if (bmeva != NULL && bmeva->total_week >= 5)
					fprintf(fp_p, "\033[1;31m%s\033[m", 
							result);
				else
...
bm_init()
{
	...
	for (i = 0; i < numboards; i++) {
		...
		for (k = 0; k < BMNUM; k++) {
			...
			strncpy(((struct bmstat *) (tmp->value))->userid, 
			...
			if (bcache[i].header.sec2[0] && 
					(strncmp(bcache[i].header.sec1, 
					bcache[i].header.sec2, 3))) {
				tmp1 = malloc(sizeof (struct hword));
				if (tmp1 == NULL) {
					errlog("Can't malloc value "
							"in bm_init!");
					exit(-1);
				}
				snprintf(tmp1->str, STRLEN - 1, "2 %s %s",
					bcache[i].header.filename,
					bcache[i].header.bm[k]);
				tmp1->value =
