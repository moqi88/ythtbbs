...
#else
	if (runtest)
		setproctitle("bbstest d %s %s", realfromhost, currentuser->userid);
	else
...
char **argv;
{
	...
	if (argc >= 3) {
		strncpy(realfromhost, argv[2], sizeof(realfromhost));
	} else {
		fromhost[0] = '\0';
	}
...
//		getdata(t_lines - 1, 0, "请输入帐号: ", uid, IDLEN + 1, DOECHO, YEA);
		getdata(t_lines - 1, 0, "请输入帐号: ", uid, IDLEN + 4, DOECHO, YEA);  //discuz account length 15
		scroll();
		{
			int l = strlen(uid);
			if (l > 0 && uid[l - 1] == '.') {
		...
		if ((check = fopen("etc/autopost", "r")) != NULL) {
			while (fgets(tmp, STRLEN, check) != NULL) {
				...
				if (dashf(fname)) {
					...
					tracelog("system post %s %s", bname,
						 ntitle);
				}
			}
			fclose(check);
...
user_login()
{
	...
	if (ansimore(fname, NA) != -1) {
		...
		while(1) {
			getdata(t_lines - 1, 0,
				"如何处理以上密码输入错误记录 (m)寄回信箱 (r)清除 (c)继续: ",
				ans, 2, DOECHO, YEA);
			if (ans[0] == 'm' || ans[0] == 'M') {
				char title[STRLEN];
...
#endif
{
	...
	init_tty();
#ifndef SSHBBS
	if (mmapfile(PASSFILE, &passwd_mf) < 0) {
		prints("mmap passwd error\n");
		refresh();
		exit(-1);
	}
	...
	getsockname(0, (struct sockaddr *)&serveraddr, &length);

	if(!strcmp(MY_BBS_ID, "YJRG") && serveraddr.sin_addr.s_addr == inet_addr("192.168.0.2"))
		strcpy(bbsdomain, "proxy.yjrg.net");
	else
		strcpy(bbsdomain, MY_BBS_DOMAIN);
