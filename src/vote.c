...
static void mk_result(int flag);
...
mk_result(int flag)
{
	...
	sprintf(fname, "vote/%s/flag.%ld", currboard,
		(long int) currvote.opendate);
	count_result(NULL);
	sprintf(sugname, "bbstmpfs/tmp/votetmp.%s.%d", currboard, uinfo.pid);
	...
	fclose(sug);
	sprintf(nname, "bbstmpfs/tmp/voteresults.%s.%d", currboard, uinfo.pid);
	...
	dele_vote();
	sprintf(fname, "vote/%s/results", currboard);
	rename(nname, fname);
	return;
VOTE_RESULT:
	ansimore(nname, YEA);
...
char *bname;
{
	...
	if (askyn("此次投票结果是否公开", NA, NA))
		...
		ball->flag &= ~VOTE_FLAG_OPENED;
	if (askyn("此次投票是否限制穿梭IP", NA, NA))
		ball->flag |= VOTE_FLAG_LIMITIP;
	else
		ball->flag &= ~VOTE_FLAG_LIMITIP;
	if (askyn("此次投票是否显示投票者完整IP", NA, NA))
		ball->flag |= VOTE_FLAG_SHOWIP;
	else
		ball->flag &= ~VOTE_FLAG_SHOWIP;
	ball->flag &= ~VOTE_FLAG_LIMITED;
	if (USERPERM(currentuser, PERM_SYSOP)) {
		if (askyn
		    ("此次投票是否设定投票人名单 (需要先由系统维护生成名单)", NA,
		     NA))
			ball->flag |= VOTE_FLAG_LIMITED;
		else
			ball->flag &= ~VOTE_FLAG_LIMITED;
	...
	if(USERPERM(currentuser, PERM_SYSOP)) {
		if (askyn("此次投票是否保存投票信息", NA, NA))
			ball->flag |= VOTE_FLAG_SAVELOG;
		else
			ball->flag &= ~VOTE_FLAG_SAVELOG;
	}
	if (askyn("此次投票是否可实时查看结果", NA, NA))
		ball->flag |= VOTE_FLAG_POLL;
	else
		ball->flag &= ~VOTE_FLAG_POLL;
	if (askyn("投票选项是否为图片", NA, NA)) {
		ball->flag |= VOTE_FLAG_IMAGE;
		getdata(cur_ln, 0, "设定该投票使用的图片目录序号:", buf, 2, DOECHO, YEA);
		ball->imagedir = atoi(buf);
	} else
		ball->flag &= ~VOTE_FLAG_IMAGE;
...
int num;
{
	...
	if (currentuser->firstlogin >= currvote.opendate - 1 * 86400) {
		prints
		    ("对不起, 开这个投票时您注册还没满 1 天, 没有资格参加哦!\n");
		pressanykey();
		return;
	}
	...
	if ((currvote.flag & VOTE_FLAG_OPENED) || 
			(currvote.flag & VOTE_FLAG_SAVELOG) || 
			(currvote.flag & VOTE_FLAG_SHOWIP)) {
		if ((currvote.flag & VOTE_FLAG_OPENED) || 
				(currvote.flag & VOTE_FLAG_SAVELOG))
			prints("请注意：该投票结束后可能公布投票信息。\n\n");
		if ((currvote.flag & VOTE_FLAG_SHOWIP))
			prints("请注意：该投票结束后可能公布真实IP。\n");
		pressanykey();
	}
...
dele_vote()
{
	...
	while (get_record(controlfile, &tmpvote, sizeof (struct votebal), num)
		...
		if (currvote.opendate == tmpvote.opendate) {
			...
			range--;
			if(currvote.flag & VOTE_FLAG_SAVELOG) {
				sprintf(buf, "vote/%s/control.%ld", currboard,
					currvote.opendate);
				append_record(buf, &currvote, sizeof(currvote));
				continue;
			}
...
int allnum, pagenum;
{
	...
	case 'r':
		...
		if(currvote.flag & VOTE_FLAG_POLL) {
			mk_result(0);
		} else {
			clear();
			prints("该投票不允许实时查看结果。");
...
b_vote()
{
	...
	if (!USERPERM(currentuser, PERM_VOTE) || (currentuser->stay < 1800)) {
		return -1;
	}
