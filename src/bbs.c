...
/* 字体应限制在 height * width = 10 * 10 以内*/
#define CAKE_PIC_ROW	16
int
show_cake()
{
	unsigned int r, ans;
	int i, j, k, m, len, count, offset;
	int bit = 4;
	char buf[256], input[8], val[256], pic[CAKE_PIC_ROW][STRLEN];
	char *tmp, *ptr;
	...
	getrandomval(val, sizeof(val)-1);
	memset(pic, ' ', sizeof(pic));
	for(i=0, ans=0, offset=0; i < bit; i++) {
		r = *((int *)val + i);
		r = r % 10;
		ans = ans * 10 + r;
		...
		MMAP_TRY {
			...
			r = (int)*(val + offset++);
			m = r%4;
			for(j=k, count=0; count<mf.size && j<16; j++) {
				tmp = (char*)mf.ptr + count;
				if((ptr=strchr(tmp, '\n')) != NULL)
					len = ptr - tmp + 1;
				else
					len = mf.size - count;
				strncpy(&pic[j][(i*12)+m], tmp, len-1); // \n
				count += len;
			}
		}
		MMAP_CATCH {
	...
	// interrupt
	for(k = 0; k < 32; k++) { // 4*2 + 32*3 + 4 = 108, 所以val用256足够 
		r = (int)*(val + offset++);
		i = r%11;
		r = (int)*(val + offset++);
		j = r%55;
		r = (int)*(val + offset++);
		pic[i][j] = simi[r % (sizeof(simi)-1)];
	}
	...
	sprintf(buf, "请输入上面的%d位数字: ", bit);
	getdata(20, 1, buf, input, 5, DOECHO, YEA);
	if (atoi(input) != ans) {
...
char *direct;
{
	struct boardheader bh;
	struct fileheader postfile;
	char collectboard[STRLEN], old_fname[STRLEN], fname[STRLEN], buf[256],
		 filepath[STRLEN], secstr;
	int oldbnum, newbnum, ddigestmode, count, fp;
	time_t now;
	FILE *inf, *of;
	int i;
	int oldftime, newftime, tmpftime, total, retv = -1;
	struct mmapfile mf = { ptr:NULL };
	...
	if (hideboard(currboard) && strcmp(currboard + 1, "Digest")) {
		clear();
		move(4, 6);
	...
	if (!strcmp(currboard, "Digest")) {
		clear();
		move(4, 6);
		prints("已经是全站文摘版了，还要转到哪里去啊？");
		pressanykey();
		return FULLUPDATE;
	}
	oldbnum = new_search_record(BOARDS, &bh, sizeof (struct boardheader), (void *) cmpbnames, currboard);
	secstr = bh.sec1[0];
	if (oldbnum <= 0)
		return DONOTHING;
	/*if (!strcmp("Digest", currboard + 1))
		//如果已经在某个分区文摘中则将目标设为全站文摘
		strcpy(collectboard, "Digest");
	else
		sprintf(collectboard, "%cDigest", secstr);
		//如果是普通版面则将目标设为分区文摘*/
	strcpy(collectboard, "Digest");
		//直接把目标版面设为Digest
	newbnum = getbnum(collectboard);
	if (newbnum <= 0) {
		clear();
		move (4, 6);
		prints("当前讨论区所在的分区没有文摘版面，不能进行收录文摘操作。");
		pressanykey();
		return FULLUPDATE;
	}
	...
	if (!USERPERM(currentuser, PERM_SYSOP) && !clubtest(collectboard) 
			&& !clubtest("Digest")){
		clear();
		move (4, 6);
		prints("很抱歉，目前只有文摘员能收录文摘。");
		pressanykey();
		return FULLUPDATE;
	} else {
		strncpy(collectboard, "Digest", STRLEN);
		newbnum = getbnum(collectboard);
	}
	...
	//主要部分：将文摘转到文摘版
	strncpy(old_fname, fh2fname(fileinfo), sizeof (old_fname));
	sprintf(genbuf, "boards/%s/%s", currboard, old_fname);
	strsncpy(quote_file, genbuf, sizeof (quote_file));
	strncpy(quote_title, fileinfo->title, sizeof (quote_title));
	ddigestmode = digestmode;
	digestmode = 0;
	bzero(&postfile, sizeof (struct fileheader));
	now = time(NULL);
	sprintf(fname, "M.%d.A", (int) now);
	setbfile(filepath, collectboard, fname);
	count = 0;
	...
	if (fileinfo && fileinfo->accessed & FH_MATH)
		postfile.accessed |= FH_MATH;
	strsncpy(postfile.owner, fileinfo->owner, sizeof(postfile.owner));
	modify_user_mode(POSTING);
	postfile.sizebyte = numbyte(eff_size(filepath));
	if (eff_size_isjunk)
		postfile.accessed |= FH_DEL;
	strsncpy(postfile.title, fileinfo->title, sizeof (postfile.title));
	i = strlen(postfile.title) - 1;
	while(i > 0 && isspace(postfile.title[i]))
		postfile.title[i--] = 0;
	setbdir(genbuf, collectboard, digestmode);
	digestmode = ddigestmode;
	...
	if (inf == NULL || of == NULL) {
		...
		if (of)
			fclose(of);
		errlog("Collection Post error");
		return DONOTHING;
	}
	while ((count = fread(buf, 1, sizeof (buf), inf)) > 0)
		fwrite(buf, 1, count, of);
	sprintf(buf, "--\n\033[m\033[1;%2dm※ 推荐:．%s@%s．[FROM: %-.20s]\033[m\n",
		currentuser->numlogins % 7 + 31, currentuser->userid, MY_BBS_ID, fromhost);
	fputs(buf, of);
	fclose(inf);
	fclose(of);
	if (append_record(genbuf, &postfile, sizeof (struct fileheader)) == -1)
		return 1;
	updatelastpost(collectboard);
	digestmode = ddigestmode;

	//开始增加日志
	oldftime = fileinfo->filetime;
	newftime = postfile.filetime;
	now = time(NULL);
	sprintf(genbuf, "%d\t%d\t%d\t%d\t%d\t%c", (int)now, oldbnum, oldftime,
				            newbnum, newftime, secstr);
							//不管三七二十一先按照当前版面写日志
	if (!strcmp(collectboard, "Digest")) {
		//如果目标版面是全站的文摘版面则将原文件位置更正
		setbfile(buf, currboard, ".DIGESTLOG");
		if ((inf = fopen(buf, "r")) != NULL) {
			while (fscanf(inf, "%*d\t%d\t%d\t%*d\t%d\t%c", &oldbnum, &oldftime, &tmpftime, &secstr) > 0) {
				//借用一下filepath，实际上是版名
				if (tmpftime == fileinfo->filetime) {
					//正好在分区文摘的日志中找到这个文件了
					setbfile(buf, bbsinfo.bcache[oldbnum - 1].header.filename, ".DIR");
					MMAP_TRY {
						if (mmapfile(buf, &mf) == -1) {
							MMAP_UNTRY;
							return -1;
						...
						retv = Search_Bin((struct fileheader *)mf.ptr, oldftime, 0, total - 1);
					}
					MMAP_CATCH {
					}
					MMAP_END mmapfile(NULL, &mf);
					if (retv >= 0) {	 
						sprintf(genbuf, "%d\t%d\t%d\t%d\t%d\t%c", (int)now, oldbnum,
								oldftime, newbnum, newftime, secstr);
						break;
					}
				}
			}
	...
	setbfile(buf, collectboard, ".DIGESTLOG");
	addtofile(buf, genbuf);

	//通知一下转载成功了
	clear();
	move (4, 6);
	if (!strcmp(collectboard, "Digest"))
		prints("%s 已转贴至全站文摘版", postfile.title);
	else
		prints("%s 已转贴到 %c 区文摘版", postfile.title, secstr);
	pressreturn();
	return FULLUPDATE;
}
...
do_evaluate(int ent, struct fileheader *fhdr, char *direct, int mode)
{
	...
	if (cl == 1 || cl == oldstar) {
		...
		if (oldstar != 1 && oldstar != 0)
			...
			prints("这篇文章被您评价为 %d 星级。", cl - 1);
			if (strncmp(currentuser->userid, fhdr->owner, IDLEN)) {
				//sleep(1);	//延时要不要呢，好像没啥大意思，反而感觉不爽
				if( !(random()%20) )	//小菜几率其实最好改成跟时间间隔相关
/*					if (show_cake()) 
 			                	Q_Goodbye();*/  //目前没有刷评价现象，这个没有必要留着。精华值的问题要改掉，不能轻易得到。
 			        if( !(random()%5)){
					if (loaduserdata(fhdr->owner, &currdata) == -1) {
						pressanykey();
 				                return 0;
					}
					currdata.extraexp += (cl - 1);
					saveuserdata(fhdr->owner, &currdata);
					prints("作者精华值+%d。", cl - 1 );
				}
				if( !(random()%5)){
					loaduserdata(currentuser->userid, &currdata);
					currdata.extraexp++;
					saveuserdata(currentuser->userid, &currdata);
					prints("您的精华值+1。");
				}
			}
		}
...
void
board_attach_link(char *buf, int buf_len, long attachpos, char *attachname, void *arg)
{
	...
	snprintf(buf, buf_len,
		 ...
		 bbsdomain, ss, getbnum(currboard) - 1,
...
#ifndef SSHBBS
			if (seek_in_file("etc/ssh_only_boards", 
						bname)) {
				move(2, 0);
				clrtoeol();
...
#ifndef SSHBBS
			if (seek_in_file("etc/ssh_only_boards", bname)) {
				move(2, 0);
				clrtoeol();
				prints("目前本版只能通过SSH方式"
						"或HTTPS方式访问");
...
char *filepath;
{
	...
	if (!noidboard)
		...
		fprintf(fp, "\n\033[m\033[1;32m※ 来源:．%s %s．[FROM: 匿名天使的家]\033[m\n",
				MY_BBS_NAME, email_domain());
	fclose(fp);
	return;
}
...
post_article(struct fileheader *sfh)
{
	...
	post_header_retv = post_header(&header);
	if (post_header_retv == 999) {
		return 999;
	} else if (post_header_retv) {
...
/*        pressreturn() ;*/
	...
	tracelog("%s post %s %s", currentuser->userid, currboard,
		 postfile.title);
	if(replytitle) {
		sprintf(buf, "1 %s %s", currentuser->userid, sfh->owner);
		friendslog(buf);
	}
	if (!hideboard(currboard)) {
		char aibuf[512];
		log_article(&postfile, newfilepath, currboard);
		snprintf(aibuf, sizeof(aibuf), "1\t%s\t%s\t%s", 
				newfilepath, currentuser->userid, postfile.title);
		ailog(aibuf);
	}
...
//everything that should be checked before i post in boardname
int
post_check_perm(char *boardname)
{
	int bnum;
	bnum = getbnum(boardname);
	if (!haspostperm(bnum)) {
		move(3, 0);
		clrtobot();
		if (digestmode == NA) {
	...
	if (club_board(boardname, bnum)) {
		if (!clubtest(boardname)) {
			if (USERPERM(currentuser, PERM_SYSOP)) {
				if (!(bbsinfo.bcache[bnum - 1].header.flag & CLOSECLUB_FLAG)) {
					move(3, 0);
					clrtobot();
					     getdata(4, 0, "该版面为俱乐部版面，您还不是俱乐部成员，"
							     "确定要发表吗(Y/N) [N] ", genbuf, 2, DOECHO, YEA);
	...
	if (deny_me(boardname) && !USERPERM(currentuser, PERM_SYSOP)) {
		move(3, 0);
		clrtobot();
		prints
		    ("\n\n                 很抱歉，你被版主停止 POST 的权利。");
	...
	if (deny_me_global()
	    && strcmp(boardname, "sysop")
	    && strcmp(boardname, "Arbitration")
	    && !USERPERM(currentuser, PERM_SYSOP)) {
		move(3, 0);
		clrtobot();
		prints
...
char *direct;
{
	...
	if (!in_mail) {
		if (digestmode == 4 || digestmode == 5)
			return DONOTHING;
		if (!strcmp(currboard, "syssecurity") || !strcmp(currboard, "newcomers"))
			return DONOTHING;
		if (!IScurrBM && !isowner(currentuser, fileinfo))
			return DONOTHING;
		if (is1984_board(currboard))
...
int
setbmuse_post(int ent, struct fileheader *fileinfo, char *direct) {
	if (!IScurrBM) {
		return DONOTHING;
	}
	if (fileinfo->accessed & FH_BMUSE)
		tracelog("%s setbmuse %s %s %s", currentuser->userid, 
				currboard, fh2owner(fileinfo), 
				fileinfo->title);
	else
		tracelog("%s setbmuse %s %s %s", currentuser->userid, 
				currboard, fh2owner(fileinfo), 
				fileinfo->title);
	change_dir(direct, fileinfo, (void *) DIR_do_setbmuse, ent, digestmode, 
			0, NULL);
	return PARTUPDATE;
}
...
char *direct;
{
	if (!strcmp(currboard, "deleted") || !strcmp(currboard, "newcomers")
...
char *direct;
{
	...
	if (digestmode == 2 || digestmode == 3
	    ...
	    || !strcmp(currboard, "syssecurity"))
		return DONOTHING;
	if (!strcmp(currboard, "deleterequest") || !strcmp(currboard, "SYSOP_MAIL")) {
...
char *direct;
{
	...
	if (has_attach) {
		prints("http://%s/" SMAGIC "%s/con_%d_%s.htm?N=%d&T=%d",
		       bbsdomain, temp_sessionid, getbnum(currboard) - 1,
...
char *direct;
{
	...
	prints("URL 地址:\n");
	prints("http://%s/" SMAGIC "%s/%scon_%d_%s.htm?T=%d\n",
	       bbsdomain, temp_sessionid,
...
marked_mode()
{
	...
	if (digestmode == 3 || digestmode == YEA) {
		...
		ans[0] = '\0';
		getdata(t_lines - 1, 0,
			"※ 0)取消 1)文摘 2)m文章 3)防水 4)相关标题 5)同作者 6)垃圾桶 7)未读 8)被删[1]:",
			ans, 2, DOECHO, NA);
		type = atoi(ans);
		if (ans[0] == '\0')
			type = 1;
...
b_notes_edit()
{
	...
	while (1) {
		getdata(3, 0,
			"编辑或删除本讨论区的 (0)离开 (1)一般备忘录 (2)秘密备忘录 (3)WWW版面简介\n"
			"                     (4)修改模板?[1] ",
			ans, 2, DOECHO, YEA);
		ch = ans[0];
		if (ch == '0')
