...
init_data()
{
	...
	while (fgets(t, 255, fp) && counts < strlen(str)) {
		...
		if (t1 == NULL || t1[0] == '#'|| t2 == NULL || t3 == NULL)
			continue;
		if(!strncmp(t3, "127", 3) || !strcasecmp("localhost", t3))
			continue;	// disable bbsnet localhost
...
sh(int n)
{
	...
	printf
	    ("\033[22;3H\033[1;37m单位: \033[1;33m%s                   \033[22;32H\033[1;37m 站名: \033[1;33m%s              \r\n",
	     host1[n], host2[n]);
	printf
	    ("\033[1;37m\033[23;3H连往: \033[1;33m%s %d                   \033[1;1H",
	     ip[n], port[n]);
}
...
show_all()
{
	...
	printf
	    ("□□□□□□□□□□\033[1;37m   现在要连到哪里呢？（Ctrl+C退出） \033[m□□□□□□□□□□□□\r\n");
	for (n = 1; n < 23; n++)
		printf
		    ("□                                                                            □\r\n");
	printf
	    ("□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□");
	if( counts == 0 ){
		printf("\033[1;31m\033[21;3H您尚未编辑自定义穿梭列表，请在主选单 \033[1;32mI-W-6 \033[m\033[1;31m 处编辑。\033[1;33mCtrl-c退出。\033[m");
	}else {
		printf
		  ("\033[21;3H----------------------------------------------------------------------------");
		for (n = 0; n < counts; n++) {
