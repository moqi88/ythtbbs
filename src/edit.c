...
insertFromFile(struct edit *edit)
{
	...
	if (n < 0 || n > 7)
		return;
	tracelog("user %s ip %s paste from clip %d", currentuser->userid, 
			realfromhost, n);
	ans[0] = n + '0';
	ans[1] = 0;
	sethomefile(filename, currentuser->userid, "clip_");
		...
		if (isprint2(ch) || ch == '\t'
		    || (ch == KEY_ESC && KEY_ESC_arg == KEY_ESC)) {
		    	...
		    	if(ch != KEY_UP && ch != KEY_DOWN
		    		...
		    		&& ch != KEY_HOME && ch != KEY_INS
		    		&& ch != KEY_DEL && ch != KEY_END
		    		&& ch != KEY_PGUP && ch != KEY_PGDN)
		    	{
#endif
			insertChar(edit, ch);
			continue;
...
int deliver;
{
	...
	if (in_mail)
		fprintf(fp, "寄信人: %s (%s)\n", uid, uname);
	else if (noname && header.chk_anony)
		fprintf(fp, "发信人: Anonymous (我是匿名天使 - %d), "
				"信区: %s\n", uinfo.pid, currboard);
	else
