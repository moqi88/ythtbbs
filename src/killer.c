...
int
k_getdata(line, col, prompt, buf, len, echo, clearlabel)
int
 line, col, len, echo, clearlabel;
char *prompt, *buf;

{
	buf[0] = '\0';
	return getdata(line, col, prompt, buf, len, echo, clearlabel);
}

void
k_setfcolor(int i, int j)
{
} void
...
struct action party_data[] = {
	...
	{"bearhug", "热情的拥抱", ""},
	{"bless", "祝福", "心想事成"},
	{"bow", "毕躬毕敬的向", "鞠躬"}, {"bye", "看着",
					  "的背影，凄然泪下。身后的收音机传来邓丽君的歌声:\n\"何日君再来...\""},
	{"caress", "轻轻的抚摸", ""}, {"cat", "像只小猫般地依偎在",
				       "的怀里撒娇。"}, {"cringe", "向",
							 "卑躬屈膝，摇尾乞怜"},
	{"cry", "向", "嚎啕大哭"}, {"comfort", "温言安慰", ""}, {"clap", "向",
								 "热烈鼓掌"},
	{"dance", "拉着", "的手翩翩起舞"}, {"dogleg", "对", "狗腿"}, {"drivel",
								      "对著",
								      "流口水"},
	{"dunno", "瞪大眼睛，天真地问：", "，你说什麽我不懂耶... :("}, {"faint",
									"晕倒在",
									"的怀里"},
	{"fear", "对", "露出怕怕的表情"}, {"fool", "请大家注意",
					   "这个大白痴....\n真是天上少有....人间仅存的活宝....\n不看太可惜了！"},
	{"forgive", "大度的对", "说：算了，原谅你了"}, {"giggle", "对著",
							"傻傻的呆笑"}, {"grin",
									"对",
									"露出邪恶的笑容"},
	{"grrr", "对", "大声咆哮"}, {"hand", "跟", "握手"}, {"hammer",
							     "举起好大好大的铁锤！！ 哇！往",
							     "头上用力一敲！\n***************\n*  5000000 Pt *\n***************\n      | |      ★☆★☆★☆\n      | |         好多的星星哟\n      |_|"},
	{"heng", "看都不看",
	 "一眼， 哼了一声，高高的把头扬起来了,不屑一顾的样子..."}, {"hug",
								    "轻轻地拥抱",
								    ""},
	{"idiot", "无情地耻笑", "的愚昧。"}, {"kick", "把", "踢的死去活来"},
	{"kiss", "轻吻", "的脸颊"}, {"laugh", "大声嘲笑", ""}, {"lovelook",
								"拉着",
								"的手，温柔地默默对视。目光中孕含着千种柔情，万般蜜意"},
	{"nod", "向", "点头称是"}, {"nudge", "用手肘顶", "的肥肚子"}, {"oh",
								       "对",
								       "说：“哦，酱子啊！”"},
	{"pad", "轻拍", "的肩膀"}, {"papaya", "敲了敲", "的木瓜脑袋"}, {"pat",
									"轻轻拍拍",
									"的头"},
	{"pinch", "用力的把", "拧的黑青"}, {"puke", "对着",
					    "吐啊吐啊，据说吐多几次就习惯了"},
	{"punch", "狠狠揍了", "一顿"}, {"pure", "对", "露出纯真的笑容"},
	{"qmarry", "向",
	 "勇敢的跪了下来:\n\"你愿意嫁给我吗？\"\n---真是勇气可嘉啊"}, {"report",
								       "偷偷地对",
								       "说：“报告我好吗？”"},
	{"shrug", "无奈地向", "耸了耸肩膀"}, {"sigh", "对", "叹了一口气"},
	{"slap", "啪啪的巴了", "一顿耳光"}, {"smooch", "拥吻着", ""},
	{"snicker",
	 "嘿嘿嘿..的对",
	 "窃笑"},
	{"sniff", "对", "嗤之以鼻"}, {"sorry", "痛哭流涕的请求", "原谅"},
	{"spank", "用巴掌打", "的屁股"}, {"squeeze", "紧紧地拥抱著", ""},
	{"thank", "向", "道谢"}, {"tickle", "咕叽!咕叽!搔", "的痒"}, {"waiting",
								      "深情地对",
								      "说：每年每月的每一天，每分每秒我都在这里等着你"},
	{"wake", "努力的摇摇", "，在其耳边大叫：“快醒醒，会着凉的！”"},
	{"wave",
	 "对著",
	 "拼命的摇手"},
	{"welcome", "热烈欢迎", "的到来"}, {"wink", "对", "神秘的眨眨眼睛"},
	{"xixi", "嘻嘻地对", "笑了几声"}, {"zap", "对", "疯狂的攻击"}, {"inn",
									"双眼饱含着泪水，无辜的望着",
									""},
	{"mm", "色眯眯的对", "问好：“美眉好～～～～"}, {"disapp",
							 "这下没搞头啦，为什么",
							 "姑娘对我这个造型完全没反应？没办法！"},
	{"miss", "真诚的望着",
	 "：我想念你我真的想念你我太--想念你了!你相不相信?"},
	{"buypig", "指着", "：“这个猪头给我切一半，谢谢！”"}, {"rascal", "对",
								 "大叫：“你这个臭流氓！！！！！！！！！！！！！！！！”"},
	{"qifu", "小嘴一扁，对", "哭道：“你欺负我，你欺负我！！！”"}, {"wa",
									 "对",
									 "大叫一声：“哇哇哇哇哇哇酷弊了耶！！！！！！！！！！！！！！”"},
	{"libel", "喔－－！熟归熟，", "你这样乱讲话，我一样可以告你毁谤，哈！"},
...
struct action speak_data[] = {
	...
	{"tease", "嘲笑", NULL},
	{"whimper", "呜咽的说", NULL},
	{"yawn", "哈欠连天", NULL}, {"yell", "大喊", NULL}, {NULL, NULL, NULL}
};
...
send_msg(int u, char *msg)
{
	...
	for (i = 0; i <= 6; i++) {
		...
		while (strstr(buf, buf3) != NULL) {
			...
			buf2[k] = 0;
			k += 2;
			sprintf(buf, "%s\033[3%dm%s", buf2, (i > 0) ? i : 7,
				buf2 + k);
		}
	}
	while (strchr(buf, '\n') != NULL) {
	...
	j = MAX_MSG;
	if (inrooms[myroom].
	    msgs[(MAX_MSG - 1 + inrooms[myroom].msgi) % MAX_MSG][0] == 0)
		for (i = 0; i < MAX_MSG; i++)
			if (inrooms[myroom].
			    msgs[(i + inrooms[myroom].msgi) % MAX_MSG][0] ==
			    0) {
	...
	if (j == MAX_MSG) {
		...
		if (u == -1)
			...
			inrooms[myroom].msgpid[inrooms[myroom].msgi] =
			    inrooms[myroom].peoples[u].pid;
...
kill_msg(int u)
{
	...
	for (i = 0; i < MAX_PEOPLE; i++)
		if (inrooms[myroom].peoples[i].style != -1)
			if (u == -1 || i == u) {
				j = kill(inrooms[myroom].peoples[i].pid,
					 SIGUSR1);
				if (j == -1) {
					sprintf(buf, "%s掉线了",
						inrooms[myroom].peoples[i].
						nick);
					send_msg(-1, buf);
					start_change_inroom();
					inrooms[myroom].peoples[i].style = -1;
					rooms[myroom].people--;
					if (inrooms[myroom].peoples[i].
					    flag & PEOPLE_ROOMOP) {
...
//    if(r->level&currentuser.userlevel!=r->level) return 0;
	if (r->style != 1)
		return 0;
	if (r->flag & ROOM_SECRET && !USERPERM(currentuser, PERM_SYSOP))
		return 0;
	return 1;
}

int
can_enter(struct room_struct *r)
{
	if (r->style == -1)
		return 0;
...
//    if(r->level&currentuser.userlevel!=r->level) return 0;
	if (r->style != 1)
		return 0;
	if (r->flag & ROOM_LOCKED && !USERPERM(currentuser, PERM_SYSOP))
		return 0;
	return 1;
}
...
get_msgt()
{
	...
	for (i = 0; i < MAX_MSG; i++) {
		if (inrooms[myroom].
		    msgs[(i + inrooms[myroom].msgi) % MAX_MSG][0] == 0)
			break;
		k = inrooms[myroom].msgpid[(i + inrooms[myroom].msgi) %
					   MAX_MSG];
		if (k == -1 || k == uinfo.pid)
			j++;
	}
...
get_msgs(int s)
{
	...
	for (i = 0; i < MAX_MSG; i++) {
		...
		if (k == -1 || k == uinfo.pid) {
			if (j == s) {
				ss = inrooms[myroom].
				    msgs[(i + inrooms[myroom].msgi) % MAX_MSG];
				return ss;
			}
			j++;
		}
...
refreshit()
{
	...
	move(0, 0);
	prints
	    ("\033[44;33;1m 房间:\033[36m%-12s\033[33m话题:\033[36m%-40s\033[33m状态:\033[36m%6s\033[m",
	...
	prints("\033[1;33m");
	prints
	    ("╭\033[1;32m玩家\033[33m—————╮╭\033[1;32m讯息\033[33m———————————————————————————╮");
	move(t_lines - 2, 0);
	prints
	    ("╰———————╯╰—————————————————————————————╯\033[m");
	me = mypos;
	...
	for (i = 2; i <= t_lines - 3; i++) {
		...
		if (ipage + i - 2 >= 0 && ipage + i - 2 < rooms[myroom].people) {
			...
			if (inrooms[myroom].status != INROOM_STOP	//!!!!!!!!!11
			    ...
			    && (inrooms[myroom].peoples[me].
				flag & PEOPLE_KILLER
				|| inrooms[myroom].peoples[me].
				flag & PEOPLE_SPECTATOR
				|| !(inrooms[myroom].peoples[j].
				     flag & PEOPLE_ALIVE))) {
				strcat(disp, "\033[31m*");
			} else
				strcat(disp, " ");
			...
			if (inrooms[myroom].peoples[j].nick[0] == '\0')
				sprintf(buf, "%d %-9.9s", j + 1,
					inrooms[myroom].peoples[j].id);
			...
			if (inrooms[myroom].status == INROOM_DAY ||
				...
				for (i0 = 0; i0 < MAX_PEOPLE; i0++)
					if (inrooms[myroom].peoples[i0].
					    style !=
					    -1 && inrooms[myroom].peoples[i0].
					    vote ==
				...
				if (k > 0) {
					...
					buf2[k] = 0;
					strcpy(buf3, buf + k);
					sprintf(buf, "\033[4m%s\033[m%s%s",
						buf2,
						(ipage + i - 2 ==
						 selected) ? "\033[31;1m" :
						"", buf3);
		...
		if (msgst - 1 - (t_lines - 3 - i) - jpage >= 0) {
			char *ss =
			    get_msgs(msgst - 1 - (t_lines - 3 - i) - jpage);
			if (ss && !strcmp(ss, "你被踢了"))
				kicked = 1;
			if (ss)
				strcat(disp, ss);
		}
...
start_killergame()
{
	...
	for (i = 0; i < MAX_PEOPLE; i++)
		if (inrooms[myroom].peoples[i].style != -1)
			if (!
				...
				total++;
	if (total < 6) {
		send_msg(me, "\033[31;1m至少6人参加才能开始游戏\033[m");
		end_change_inroom();
		refreshit();
		return;
	}
	...
	if (totalk > total) {
		send_msg(me,
			 "\033[31;1m总人数少于要求的坏人人数,无法开始游戏\033[m");
		end_change_inroom();
		refreshit();
		return;
	}
	if (totalc == 0)
		sprintf(buf,
			"\033[31;1m游戏开始啦! 人群中出现了%d个坏人\033[m",
			totalk);

	else
		sprintf(buf,
			"\033[31;1m游戏开始啦! 人群中出现了%d个坏人, %d个警察\033[m",
			totalk, totalc);
	send_msg(-1, buf);
	for (i = 0; i < totalk; i++) {
		...
		do {
			j = rand() % MAX_PEOPLE;
		} while (inrooms[myroom].peoples[j].style == -1
			 || inrooms[myroom].peoples[j].flag & PEOPLE_KILLER
			 || inrooms[myroom].peoples[j].flag & PEOPLE_SPECTATOR);
		inrooms[myroom].peoples[j].flag |= PEOPLE_KILLER;
		send_msg(j, "你做了一个无耻的坏人");
		send_msg(j,
			 "用你的尖刀(\033[31;1mCtrl+S\033[m)选择你要残害的人吧...");
	}
	for (i = 0; i < totalc; i++) {
		...
		send_msg(j,
			 "白天请用你的警棒(\033[31;1mCtrl+T\033[m)选择你怀疑的人...");
	}
	for (i = 0; i < MAX_PEOPLE; i++)
		if (inrooms[myroom].peoples[i].style != -1)
			if (!
...
do_com_menu()
{
	...
	do {
		...
		disp[0] = '\0';
		for (i = 0; i < menust; i++)
			if (inrooms[myroom].peoples[j].flag & PEOPLE_ROOMOP
			    || i <= 3) {
				if (i == sel) {
		...
		case '\r':
			...
			case 2:
				...
				k_resetcolor();
				clrtoeol();
				k_getdata(t_lines - 1, 0, "请输入名字:", buf,
					  13, 1, 0);
				if (kicked)
					return 0;
				if (buf[0]) {
					...
					start_change_inroom();
					me = mypos;
					strcpy(inrooms[myroom].peoples[me].
					       nick, buf);
					end_change_inroom();
					kill_msg(-1);
				}
			...
			case 3:
				me = mypos;
				for (i = 0; i < MAX_PEOPLE; i++)
					if (inrooms[myroom].peoples[i].style !=
					    -1) {
						sprintf(buf, "%-12s  %s",
							inrooms[myroom].
							peoples[i].id,
			...
			case 4:
				...
				k_resetcolor();
				clrtoeol();
				k_getdata(t_lines - 1, 0, "请输入话题:", buf,
					  31, 1, 1);
				if (kicked)
					return 0;
				if (buf[0]) {
			...
			case 5:
				...
				k_resetcolor();
				clrtoeol();
				k_getdata(t_lines - 1, 0,
					  "请输入房间最大人数:", buf, 30, 1, 1);
				if (kicked)
					return 0;
				if (buf[0]) {
					i = atoi(buf);
			...
			case 6:
				...
				k_resetcolor();
				clrtoeol();
				k_getdata(t_lines - 1, 0, "请输入要踢的id:",
					  buf, 30, 1, 1);
				if (kicked)
					return 0;
				if (buf[0]) {
					for (i = 0; i < MAX_PEOPLE; i++)
						if (inrooms[myroom].peoples[i].
							...
							if (!strcmp
								...
								break;
					if (!strcmp
					    (inrooms[myroom].peoples[i].id, buf)
					    && inrooms[myroom].peoples[i].
					    pid != uinfo.pid) {
						inrooms[myroom].peoples[i].
						    flag &= ~PEOPLE_ALIVE;
...
join_room(int w, int spec)
{
	...
	if (rooms[myroom].people == 0
	    && !strcmp(rooms[myroom].creator, currentuser->userid))
	...
	end_change_inroom();
	kill_msg(-1);
	sprintf(buf, "\033[36m%s进入房间\033[0m", currentuser->userid);
	send_msg(-1, buf);
	room_refresh(0);
	while (1) {
		...
		do {
			int ch;
			ch = -k_getdata(t_lines - 1, 0, "输入:", buf, 70, 1, 1);
			if (rooms[myroom].style != 1)
				kicked = 1;
			if (kicked)
				goto quitgame;
			if (ch == KEY_UP) {
				...
				if (inrooms[myroom].peoples[me].
				    flag & PEOPLE_TESTED) {
					send_msg(me,
						 "\033[31;1m本轮你已经侦查过了\033[m");
					refreshit();
					continue;
				}
				if (inrooms[myroom].peoples[sel].
				    flag & PEOPLE_SPECTATOR)
					send_msg(me,
						 "\033[31;1m此人是旁观者\033[m");

				else if (!
					 (inrooms[myroom].peoples[sel].
					  flag & PEOPLE_ALIVE))
					send_msg(me,
						 "\033[31;1m此人已死\033[m");

				else if ((inrooms[myroom].peoples[sel].
					  flag & PEOPLE_KILLER)) {
					inrooms[myroom].peoples[me].flag |=
					    PEOPLE_TESTED;
					send_msg(me,
						 "\033[31;1m经过你的侦测, 发现此人是坏人!!!\033[m");
				}
						...
						if (inrooms[myroom].
							...
							if (inrooms[myroom].
								...
								for (i = 0;
								     ...
								     i++)
									if (inrooms[myroom].peoples[i].style != -1)
										if (inrooms[myroom].peoples[i].flag & PEOPLE_KILLER || inrooms[myroom].peoples[i].flag & PEOPLE_SPECTATOR)
											send_msg
											    (i,
											     buf);
							}
...
(inrooms
 [myroom].
 peoples[i].flag & PEOPLE_KILLER || inrooms[myroom].status == INROOM_DAY)) {
										for (j = 0; j < MAX_PEOPLE; j++)
											if (inrooms[myroom].peoples[j].style != -1)
 ...
 peoples[i].flag & PEOPLE_KILLER || inrooms[myroom].status == INROOM_DAY))
							...
							if (j || t1 - t2 > t3) {
								...
								for (i = 0;
								     ...
								     i++)
									if (inrooms[myroom].peoples[i].style != -1)
										if (!(inrooms[myroom].peoples[i].flag & PEOPLE_SPECTATOR) && inrooms[myroom].peoples[i].flag & PEOPLE_ALIVE && (inrooms[myroom].peoples[i].flag & PEOPLE_KILLER || inrooms[myroom].status == INROOM_DAY)) {
											for (j = 0; j < MAX_PEOPLE; j++)
												if (inrooms[myroom].peoples[j].style != -1)
													if (inrooms[myroom].peoples[j].pid == inrooms[myroom].peoples[i].vote)
														inrooms
								    ...
								    INROOM_NIGHT)
								{
									for (j
									     ...
									     j++)
										if (inrooms[myroom].peoples[j].style != -1)
											if (inrooms[myroom].peoples[j].flag & PEOPLE_KILLER || inrooms[myroom].peoples[j].flag & PEOPLE_SPECTATOR)
												send_msg
												    (j,
												     buf);
								}
								...
								for (i = 0;
								     ...
								     i++)
									if (inrooms[myroom].peoples[i].style != -1)
										if (!(inrooms[myroom].peoples[i].flag & PEOPLE_SPECTATOR) && inrooms[myroom].peoples[i].flag & PEOPLE_ALIVE) {
											sprintf
											    (buf,
											     "%s的投票数: %d 票",
											     inrooms
								...
								if (!ok) {
									    ...
									    INROOM_DAY)
									{
										...
										for (j = 0; j < MAX_PEOPLE; j++)
											if (inrooms[myroom].peoples[j].style != -1)
												if (inrooms[myroom].peoples[j].flag & PEOPLE_KILLER && inrooms[myroom].peoples[j].flag & PEOPLE_ALIVE) {
													sprintf
													    (buf,
													     "原来%s是坏人!",
													     inrooms
										...
										for (i = 0; i < MAX_PEOPLE; i++)
											if (inrooms[myroom].peoples[i].style != -1)
												if (inrooms[myroom].peoples[i].flag & PEOPLE_KILLER && inrooms[myroom].peoples[i].flag & PEOPLE_ALIVE)
													send_msg
													    (i,
													     "请抓紧你的宝贵时间用\033[31;1mCtrl+S\033[m杀人...");
									}
		...
		if (msg[0] == '/' && msg[1] == '/') {
			...
			for (i = 0;; i++) {
				...
				if (!strcasecmp(party_data[i].verb, buf)) {
					k = 0;
					sprintf(buf,
						"%s \033[1;37m%s\033[0m %s",
						party_data[i].part1_msg,
						buf2[0] ? buf2 : "大家",
			...
			if (k)
				continue;
			strcpy(buf2, buf);
			sprintf(buf, "\033[1m%d %s\033[m %s", me + 1,
				inrooms[myroom].peoples[me].nick, buf2);
		}

		else {
			strcpy(buf2, buf);
			sprintf(buf, "%d %s: %s", me + 1,
				inrooms[myroom].peoples[me].nick, buf2);
		}
		if (inrooms[myroom].status == INROOM_NIGHT) {
	...
	end_change_inroom();
	if (inrooms[myroom].peoples[me].flag & PEOPLE_KILLER)
		sprintf(buf, "\033[34m杀手%s潜逃了...\033[0m",
			inrooms[myroom].peoples[me].id);

	else
		sprintf(buf, "\033[36m%s离开房间\033[0m",
			inrooms[myroom].peoples[me].id);
	send_msg(-1, buf);
      quitgame2:kicked = 0;
	k_getdata(t_lines - 1, 0, "寄回本次全部信息吗?[y/N]", buf3, 3, 1, 1);
	if (toupper(buf3[0]) == 'Y') {
...
static int
room_list_refresh(struct _select_def *conf)
{
	clear();
	docmdtitle("[游戏室选单]",
		   "  退出[\033[1;32m←\033[0;37m,\033[1;32me\033[0;37m] 进入[\033[1;32mEnter\033[0;37m] 选择[\033[1;32m↑\033[0;37m,\033[1;32m↓\033[0;37m] 添加[\033[1;32ma\033[0;37m] 加入[\033[1;32mJ\033[0;37m] \033[m      作者: \033[31;1mbad@smth.org\033[m");
	move(2, 0);
	prints("\033[0;1;37;44m    %4s %-14s %-12s %4s %4s %6s %-20s\033[m",
	       "编号", "游戏室名称", "创建者", "人数", "最多", "锁隐旁",
	       "话题");
	clrtoeol();
	k_resetcolor();
	update_endline();
...
static int
room_list_show(struct _select_def *conf, int i)
{
	struct room_struct *r;
	int j = room_get(i - 1);
	if (j != -1) {
		r = rooms + j;
		prints("  %3d  %-14s %-12s %3d  %3d  %2s%2s%2s %-36s", i,
		       r->name, r->creator, r->people, r->maxpeople,
		       (r->flag & ROOM_LOCKED) ? "√" : "",
		       (r->flag & ROOM_SECRET) ? "√" : "",
...
static int
room_list_select(struct _select_def *conf)
{
	...
	k_getdata(0, 0, "是否以旁观者身份进入?\033[m[y/N]", ans, 3, 1, 1);
...
static int
room_list_k_getdata(struct _select_def *conf, int pos, int len)
{
	clear_room();
...
static int
room_list_prekey(struct _select_def *conf, int *key)
{
	switch (*key) {
	case 'e':
	case 'q':
...
static int
room_list_key(struct _select_def *conf, int key)
{
	struct room_struct r, *r2;
	int i, j;
	char name[40], ans[4];
	...
	case 'J':
		...
		k_getdata(0, 0, "是否以旁观者身份进入?\033[m[y/N]", ans, 3, 1,
			  1);
		if (toupper(ans[0]) == 'Y' && r2->flag & ROOM_DENYSPEC
		    && !USERPERM(currentuser, PERM_SYSOP)) {
			move(0, 0);
...
void *
killer_get_shm(key_t shmkey, size_t shmsize, int *create)
{
	int shmid;
	void *shmptr = (void *) -1;
...
killer_main()
{
	...
	void *shm, *shm2;
	shm =
	    killer_get_shm(getBBSKey(GAMEROOM_SHM),
			   sizeof (struct room_struct) * MAX_ROOM, &i);
	rooms = shm;
	if (i) {
		for (i = 0; i < MAX_ROOM; i++) {
			rooms[i].style = -1;
	...
	shm2 =
	    killer_get_shm(getBBSKey(KILLER_SHM),
			   sizeof (struct inroom_struct) * MAX_ROOM, &i);
	inrooms = shm2;
	if (i) {
		for (i = 0; i < MAX_ROOM; i++)
			inrooms[i].w = 0;
