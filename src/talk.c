...
char q_id[IDLEN + 2];
{
	...
	move(1, 0);
	prints
	    ("\033[m\033[1m%s \033[m(\033[1;32m%s\033[m) 共上站 \033[1;32m%d\033[m 次，发文 \033[1;32m%d\033[m 篇",
	...
	if (is_inprison(lookupuser->userid)) {
		strcpy(genbuf, Ctime(lookupuser->lastlogin));
		prints("\n在监狱服刑，上次放风时间[\033[1;32m%s\033[m]\n", genbuf);
		if (uinfo.mode != LUSERS && uinfo.mode != LAUSERS
	...
	if (!USERPERM(currentuser, PERM_SYSOP))
		in.s_addr = lookupuser->lasthost & IPMASK;
	else
		in.s_addr = lookupuser->lasthost;
	prints("\n上次在 [\033[1;32m%s\033[m] 从 [\033[1;32m%s\033[m] 到本站一游。信箱：[\033[1;5;32m%2s\033[m]\n",
...
#endif
	prints
	    ("经验值：[\033[1;32m%d+%d\033[m](\033[1;33m%s\033[m) 表现值：[\033[1;32m%d\033[m](\033[1;33m%s\033[m) 生命力：[\033[1;35m%d\033[m]\n",
	     (lookupuser->exp_group=='U')?0:countexp(lookupuser, 0), (lookupuser->exp_group=='U')?0:countexp(lookupuser, 1), cuserexp(lookupuser->exp_group, countexp(lookupuser,2)),
	     countperf(lookupuser), cperf(countperf(lookupuser)),countlife(lookupuser));
	if( USERPERM(lookupuser, PERM_SYSOP))
		prints("职务：\033[1;33m站务总管\033[m  ");
	else if (USERPERM(lookupuser, PERM_ARBITRATE))
		prints("职务：\033[1;33m仲裁\033[m  ");
	else if (USERPERM(lookupuser, PERM_SPECIAL4))
		prints("职务：\033[1;33m分区站务\033[m  ");
	else if (USERPERM(lookupuser, PERM_EXT_IDLE))
		prints("职务：\033[1;33m执法站务\033[m  ");
	else if (USERPERM(lookupuser, PERM_SPECIAL5))
		prints("职务：\033[1;33m美工站务\033[m  ");
	else if (USERPERM(lookupuser, PERM_SPECIAL6))
		prints("职务：\033[1;33m立法会\033[m  ");
#ifdef ENABLE_BLOG
	else if (USERPERM(lookupuser, PERM_SPECIAL7))
		prints("职务：\033[1;33mBLOG 管理员\033[m  ");
#endif
	if (lookupuser->userlevel & PERM_BOARDS) {
		prints("担任版务：\033[1;36m");
		apply_boards(bm_printboard, lookupuser->userid);
		prints("\033[m\n");
	}
	else if(USERPERM(lookupuser, PERM_SPECIAL4) || USERPERM(lookupuser, PERM_EXT_IDLE)|| USERPERM(lookupuser, PERM_SPECIAL5) || USERPERM(lookupuser, PERM_SPECIAL6) || USERPERM(lookupuser, PERM_SYSOP))
		prints("\n");
	if(!t_search_ulist(tuid))
		prints("目前不在站上，上次离站时间：[\033[1;36m%s\033[0m]\n", (lookupuser->lastlogout >= lookupuser->lastlogin) ? Ctime(lookupuser->lastlogout) : "可能因不正常断线不详" );
	else
		prints("\n");
	if (uinfo.mode != LUSERS && uinfo.mode != LAUSERS
	    && uinfo.mode != FRIEND && uinfo.mode != GMENU) {
...
#ifdef ENABLE_BLOG
		move(t_lines - 2, 0);
		if (lookupuser->hasblog) {
			if(isalpha(lookupuser->userid[0]))
				prints("\033[1;37m博客地址: [\033[1;32m http://%s.yilubbs.com \033[1;37m]", lookupuser->userid);
			else
				prints("\033[1;37m博客地址: [\033[1;32m http://yilubbs.com.net/" SMAGIC "/blog?U=%d \033[1;37m]", getuser(lookupuser->userid, NULL));
		}
#endif
		move(t_lines - 1, 0);
		prints
		    ("\033[1;37m寄信[\033[1;32mm\033[1;37m] 送讯息[\033[1;32ms\033[1;37m] 加,减好友[\033[1;32mo\033[1;37m,\033[1;32md\033[1;37m] 朋友圈[\033[1;32mp\033[1;37m] 看文集[\033[1;32mx\033[1;37m]");
		clrtoeol();
		resetcolor();
		ch = igetkey();
		switch (toupper(ch)) {
	...
	uinfo.destuid = 0;
	if (strcmp("guest", currentuser->userid)) {
		sprintf(genbuf, "1 %s %s", currentuser->userid, uident);
		friendslog(genbuf);
	}
	return 0;
}
...
char *uident;
{
	...
	if (n != -1) {
		...
		if(friendflag)
			sprintf(genbuf, "300 %s %s", currentuser->userid, uident);
		else
			sprintf(genbuf, "-300 %s %s", currentuser->userid, uident);
	        friendslog(genbuf);			
	} else
...
//------------------------朋友圈----------------------------//a
static void show_friends_welcome() {
	move(0, 0);
        prints( "\033[1;44m\033[1;33m                            欢迎来到朋友圈"
		"                                            \033[0m");
	return;
}

static int query_friends(char *userid, char *title, char *msg, char *file)
{
	int count=0, max=10;
...
static void do_afriend(char *userid) {
	char fpath[256], title[256], buf[STRLEN];
	char ch;

	clear();
	show_friends_welcome();
	move(1, 0);
	prints( "\033[0m要查看\033[1;32m%s\033[0m的: "
		"(i)偶像 (f)粉丝 (d)圈子 (g)去路径 (c)来路径 (e)退出", userid);
	while(1) {
		move(3, 0);
		clrtobot();
		ch = _tolower(igetkey());
		...
		case 'f':
			sprintf(title, "--------------------%s的%s--------------------",
					userid, ch == 'i' ? "偶像" : "粉丝");
			sprintf(fpath, FRIENDS_TMPDIR "/%s.%d", userid, ch == 'i' ? 1: 0);
			sprintf(buf, "%c %s", ch, userid);
			query_friends(userid, title, buf, fpath);
			break;
		case 'd':
			sprintf(title, "--------------------%s的圈子--------------------", userid);
			sprintf(fpath, FRIENDS_TMPDIR "/%s.div", userid);
			sprintf(buf, "d %s", userid);
			query_friends(userid, title, buf, fpath);
			break;
		case 'g':
			sprintf(title, "--------------------您到%s的最短路径--------------------", userid);
			sprintf(fpath, FRIENDS_TMPDIR "/%s.%s.path", currentuser->userid, userid);
			sprintf(buf, "p %s %s", currentuser->userid, userid);
			query_friends(userid, title, buf, fpath);
			break;
		case 'c':
			sprintf(title, "--------------------%s到您的最短路径--------------------", userid);
			sprintf(fpath, FRIENDS_TMPDIR "/%s.%s.path", userid, currentuser->userid);
			sprintf(buf, "p %s %s", userid, currentuser->userid);
			query_friends(userid, title, buf, fpath);
			break;
		case 'e':
			return;
...
static void show_friends_pp() {
	move(16, 0);
	prints( "\033[1;32m朋友圈申明\033[0m\n"
		"    1. 朋友圈旨在趣味，完全由程序统计生成，无任何人为因素。\n"
		"    2. 程序只统计用户行为，并不收集/分析/泄漏信息内容。\n"
		"    3. 朋友圈的统计结果并不代表该用户或站点的意志。\n"
		"    4. 用户可自愿选择加入或退出朋友圈。");
	return;
}


static void show_friends_top() {
	int r;
	struct {
		char fname[32];
		char title[32];
	} tops[] =	{{"top10.idolA", "十大大众情人"},
			 {"top10.fansA", "十大狂热追星"},
			 {"top10.idolB", "十大实力偶像"},
			 {"top10.fansB", "十大忠诚粉丝"},
			 {"top10.bigdiv", "十大势力圈子"}
	...
	r = random() % 5; // must be according with tops' boudary

	sprintf(fpath, FRIENDS_TMPDIR "/%s", tops[r].fname);
	move(4, 0);
	prints("-------------------------\033[1;36m%s\033[0m-------------------------", tops[r].title);
	ansimore2(fpath, 0, 5, 10);
	return;
}

	
int Friends(char *userid) {
	char uident[IDLEN+1];

	if(!strcmp(userid, "G")) { //call from talk menu
		modify_user_mode(QUERY);
		*uident = '\0';
	} else //call from ctrl-a with userid
