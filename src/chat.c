...
char *arg;
{
	...
	if (is_inprison(lookupuser->userid)) {
		...
		sprintf(msg,
			...
			genbuf,
			(lookupuser->lasthost == 0 ? "(不详)" : inet_ntoa(in)));
	}
	...
	sprintf(msg,
		...
		countlife(lookupuser));
	printchatline(msg);
	sprintf(msg, "\033[1;37m目前%s站上\033[m",
		(t_search(userid, NA, 1) != NULL) ? "在" : "不在");
	printchatline(msg);
}
...
/*        if(!(USERPERM(currentuser, PERM_SYSOP)||USERPERM(currentuser, PERM_SEECLOAK)))
	...
	if (recflag == 0) {
		...
		fprintf(rec, "结束时间：%s\n", ctime(&now));
		fclose(rec);
		system_mail_file(fname, currentuser->userid, "录音结果", currentuser->userid);
		unlink(fname);
...
char *arg;
{
	...
	*msgstr = 0;
	...
	if (do_sendmsg(uentp->userid, uentp, msgstr, 2, 0) != 1) {
		sprintf(showstr, "\033[1m无法发消息给 %s \033[m",
			uentp->userid);
