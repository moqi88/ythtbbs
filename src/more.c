...
{
	int i, w, in_esc = 0, eschtml = 0, db = 0, lastspace = 0, asciiart = 0;
	char *p = p0;
	if (size <= 0) {
	...
	if (*s == size)
		return 0;
	if (oldty != LINE_QUOTA_NOCF && oldty != LINE_NORMAL_NOCF && *ty == LINE_NORMAL && size > 10
	...
	if (oldty != LINE_QUOTA_NOCF && oldty != LINE_NORMAL_NOCF && *ty == LINE_NORMAL && size > 19
...
next_MemMoreLines(struct MemMoreLines *l)
{
	...
	if (l->curr_line + 1 >= l->start + l->num) {
		...
		n = (l->start + l->num) % 100;
		l->line[n] = p0;
		if (measure_line(p0, l->size - (p0 - l->ptr), &l->len[n], &l->s[n],
			     oldty, &l->ty[n]) < 0)
			return -1;
		l->num++;
...
mem_printline(char *ptr, int len, char *fn, char ty, char *begin)
{
	...
	if (!strncmp(ptr, "□ 引用", 7) || !strncmp(ptr, "==>", 3)
		...
		int attlen;
		isattached = 1;
		strsncpy(attachname, ptr + (ty == LINE_ATTACHMENT_BASE64 ? 10 : 18),
			 sizeof (attachname));
		sizestr[0] = 0;
		if (ty == LINE_ATTACHMENT_BINARY) {
			...
			if (str) {
				...
				if (str) {
					...
					if (attlen <= 10240)
						snprintf(sizestr, sizeof(sizestr), "(%d 字节)", attlen);
					else
						snprintf(sizestr, sizeof(sizestr), "(%d K字节)", attlen / 1024);
				}
			} 
		}
		p = strchr(attachname, '\n');
		...
		prints (" %s %s 链接: \n", attachname, sizestr);
		return;
	} else if (ty == LINE_ATTACHMENT_BINARY_LINK || ty == LINE_ATTACHMENT_BASE64_LINK) {
		char attachname[41], *p, *str;
		strsncpy(attachname, ptr + (ty == LINE_ATTACHMENT_BASE64 ? 10 : 18),
			 sizeof (attachname));
		str = strchr(ptr + 18, '\n');
		if (current_attach_link && ty == LINE_ATTACHMENT_BINARY_LINK && str) {
			char slink[256];
		...
		if (!strncmp(fn, "boar", 4)) {
			...
			type = 3;
		q = strrchr(fn, '/') + 1;
		if(type == 1)	//只有普通阅读模式显示
			prints("全文链接: \033[4m");
		switch (type) {
		case 1:
			if (digestmode == YEA)
				prints("http://%s/" SMAGIC
				       "%s/gcon?B=%d&F=%s", bbsdomain,
				       temp_sessionid, getbnum(currboard) - 1,
				       q);
			else
				prints("http://%s/HT/con_%d_%s.htm", bbsdomain, getbnum(currboard) - 1, q);
			break;
		case 2:
			if (0)
				prints("http://%s/" SMAGIC "%s/anc?path=%s",
				       bbsdomain, temp_sessionid, q);
			break;
		case 3:
		...
		case 4:
			prints("http://%s/" SMAGIC "%s/c1?T=%d&F=%s",
			       bbsdomain, temp_sessionid, type, fn + 13);
			break;
		case 5:
			prints("http://%s/" SMAGIC "%s/c1?T=%d&F=%s",
			       bbsdomain, temp_sessionid, type, fn + 20);
			break;
		default:
			break;
		}
