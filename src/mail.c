...
static int check_group_mail_perm();
...
/*add by KCN :) */
	...
	if (internet_mail) {
		...
		sprintf(genbuf, "4 %s %s", currentuser->userid, userid);
		friendslog(genbuf);
		in_mail = save_in_mail;
		return 0;
	}
...
g_send()
{
	...
	if ((limit = check_group_mail_perm()) == 0) {
		pressreturn();
		return 0;
	}
	limit = maxrecp > limit ? limit : maxrecp;
	...
	*quote_file = '\0';
	...
	while (1) {
		if (cnt > limit) {
			move(2, 0);
			prints("目前您可以寄信给 \033[1m%d\033[m 人", limit);
		}
		if (!keepgoing) {
			getdata(0, 0,
		...
		case 'i':
			n = 0;
			clear();
			for (i = cnt; i < limit && n < uinfo.fnum; i++) {
				move(2, 0);
				clrtoeol();
...
static int check_group_mail_perm() {
	if(currentuser->firstlogin + 7 * 86400 > time(NULL)) {
		clear();
		prints("您的注册天数尚未达到7天，不能发群体信件");
		return 0;
	}
	return (currentuser->numlogins / 10) + (currentuser->stay / 86400) + 1;
}
...
ov_send()
{
	...
	clrtobot();
	move(2, 0);
	prints("寄信给好友名单中的人，目前您可以寄给前 \033[1m%d\033[m 位。\n", limit);
	if (uinfo.fnum <= 0) {
		prints("你并没有设定好友。\n");
		pressanykey();
		clear();
	...
	all = (uinfo.fnum >= limit) ? limit : uinfo.fnum;
	for (i = 0; i < all; i++) {
...
m_cancel_1(struct fileheader *fh, char *receiver)
{
	...
	if (YEA == askyn(buf, NA, NA)) {
		...
		now = time(0);
		sprintf(buf, MY_BBS_HOME "/etc/cancel_mail");
		if (dashf(buf)) {
			fp1 = fopen(buf, "r+");
			while (fgets(buf, sizeof(buf), fp1) != NULL) {
				if (strstr(buf, "$userid1") != NULL) {
					sprintf(tmpbuf, "%-12s", receiver);
					strncpy(strstr(buf, "$userid1"), tmpbuf, 12);
					fputs(buf, fp);
				} else if (strstr(buf, "$userid2") != NULL) {
					sprintf(tmpbuf, "%12s", currentuser->userid);
					strncpy(strstr(buf, "$userid2"), tmpbuf, 12);
					fputs(buf, fp);
				} else if (strstr(buf, "$time") != NULL) {
					strncpy(strstr(buf, "$time"), Ctime(now), strlen(Ctime(now)));
					fputs(buf, fp);
				} else
					fputs(buf, fp);
			}
