...
x_edits()
{
	...
	if( ch == 5 ) {
		move(5,0);
		prints("自定义穿梭格式如下:\n\n"
			"\033[1;32m站点描述  站名  域名或者ip  端口<可不写,则默认23>\033[m\n\n"
			"每一项中间需要用空格或者TAB隔开\n\n"
			"如：\n\033[1;35m凝聚华人力量  一路BBS  yilubbs.net 23\033[m\n");
		pressanykey();
	}
...
a_edits()
{
	...
	int ch, num, confirm, maxpos;
	static struct edit_file {
		char *file_name;		//档案位置
		char *file_explain;		//档案说明
		int file_perm;			//编辑权限
		int pos;				//在选单中的位置-1
	} e_files[] = {
		{"Welcome", "TELNET特殊进站[废]", PERM_WELCOME, },
		{"Welcome2", "TELNET进站公告", PERM_WELCOME, },
		{"issue", "TELNET进站画面", PERM_WELCOME, },
		{"logout", "TELNET离站画面", PERM_WELCOME, },
		{"movie", "TELNET活动看版", PERM_WELCOME, },
		{"endline", "TELNET屏幕底线", PERM_WELCOME, },
		{"../vote/notes", "公共备忘录", PERM_WELCOME, },
		{"../0Announce/focus", "WEB导读公告", PERM_WELCOME, },
		{"../0Announce/boa_hot", "WEB导读热点", PERM_WELCOME, },
		{"../0Announce/hotboards", "WEB推荐版面", PERM_WELCOME, }, 
		{"../0Announce/announce", "WEB首页公告", PERM_WELCOME, },
		{"../wwwtmp/digestcolum", "文摘系统配置", PERM_WELCOME, },
		{"menu.ini", "menu.ini[慎!]", PERM_SYSOP, },
		{"badname0", "不可注册之ID", PERM_ACCOUNTS, },
		{"badname", "ID不可包含之字串", PERM_ACCOUNTS, },
		{"../.bad_email", "不可确认之E-mail", PERM_ACCOUNTS, },
		{"../.bansite", "不可上站之IP", PERM_ACCOUNTS, },
		{"../.blockmail", "拒收E-mail黑名单", PERM_ACCOUNTS, },
		{"autopost", "每日自动送信档", PERM_SYSOP, },
		{"junkboards", "不算POST数版面", PERM_SYSOP, },
		{"notop10boards", "不上十大版面", PERM_SYSOP, },
		{"noboaboards", "不上导读版面", PERM_SYSOP, },
		{"filtertitle", "十大及导读过滤标题", PERM_SYSOP, },
		{"untrust", "不信任IP列表", PERM_SYSOP, },
		{"bbsnet.ini", "穿梭列表", PERM_SYSOP, },
		{"../ftphome/ftp_adm", "FTP管理员名单", PERM_SYSOP, },
		{"badwords", "全站过滤词汇", PERM_SYSOP, },
		{"sbadwords", "政治版附加过滤词", PERM_SYSOP, },
		{"pbadwords", "报警词汇", PERM_SYSOP, },
		{"../inndlog/newsfeeds.bbs", "cn.bbs转信版和新闻组对应", PERM_SYSOP, },
		{"spec_site", "穿梭IP限制次数", PERM_SYSOP, },
		{"secmlist", "区长名单", PERM_SYSOP, },
		{"bmstand", "版务考勤标准", PERM_SYSOP},
		{"bmhelp", "版务上任手册", PERM_SYSOP, },
		{"s_fill2", "注册用户欢迎信", PERM_SYSOP, },
		{"autocross", "自动转载文章规则", PERM_SYSOP, },
		{"notepad", "留言版",  PERM_EXT_IDLE, },
		{NULL, NULL, PERM_SYSOP, }
	};
	modify_user_mode(ADMIN);
	if (!check_systempasswd()) {
		return;
	...
	prints("编修系统档案\n\n");
	ch = 1;		//档案前面的标号
	for (num = 0; e_files[num].file_name != NULL; num ++) {
		if (!USERPERM(currentuser, PERM_SYSOP) && 
				!USERPERM(currentuser, e_files[num].file_perm))
					//没权限编辑就算了
			continue;
		if (ch >= 20)
			move(ch - 20 + 2, 39);
		prints("[%2d] %s\n", ch, e_files[num].file_explain);
		e_files[num].pos = ch;	//记录一下档案前面的编号
		ch++;
	}
	if (ch >= 20)
		move(ch - 20 + 2, 39);
	prints("[%2d] 都不想改\n", ch);
	maxpos = ch;					//记录一下最大的档案位置

	getdata(t_lines - 1, 0, "你要编修哪一项系统档案: ", ans, 3, DOECHO,
		YEA);
	ch = atoi(ans);		//现在ch用来表示输入的答案
	if (!isdigit(ans[0]) || ch <= 0 || ch >= maxpos || ans[0] == '\n'
	    || ans[0] == '\0')
		return;
	for (num = 0; e_files[num].file_name != NULL; num ++) {
		if (e_files[num].pos != ch)
			continue;
		ch = num;		//将输入的答案转为在所有档案中实际的位置
		break;
	}
	if (ch == maxpos)	//如果是在最大位置就直接返回，不要编辑e_files[maxpos]
		return;
	sprintf(buf2, "etc/%s", e_files[ch].file_name);
	move(3, 0);
	clrtobot();
	sprintf(buf, "(E)编辑 (D)删除 %s? [E]: ", e_files[ch].file_explain);
	getdata(3, 0, buf, ans, 2, DOECHO, YEA);
	if (ans[0] == 'D' || ans[0] == 'd') {
		...
		{
			char secu[STRLEN];
			sprintf(secu, "删除系统档案：%s", e_files[ch].file_explain);
			securityreport(secu, secu);
		}
		unlink(buf2);
		move(5, 0);
		prints("%s 已删除\n", e_files[ch].file_explain);
		pressreturn();
		clear();
		return;
	}
	...
	clear();
	if (retv != REAL_ABORT) {
		prints("%s 更新过", e_files[ch].file_explain);
		{
			char secu[STRLEN];
			sprintf(secu, "修改系统档案：%s", e_files[ch].file_explain);
			securityreport(secu, secu);
		}
	}
	pressreturn();
...
x_dict()
{
	...
	if (dashf(buf)) {
		...
		if (askyn("要将结果寄回信箱吗", NA, NA) == YEA)
			mail_file(buf, currentuser->userid, "字典查询结果", currentuser->userid);
		unlink(buf);
	}
}
#else
...
ent_bnet(char *cmd)
{
	...
	if (currentuser->stay < 60 * 60 * 0.16) {
		move(9, 0);
		clrtobot();
		prints("您目前的上站时间尚不足十分钟，还是多在本站看看吧！");
		pressanykey();
		return -1;
	} else if (count_uindex_bbsnet(usernum)>2) {
		move(9, 0);
		clrtobot();
		prints("您已经有其他窗口正在使用BBSNET服务了。");
		pressanykey();
		return -1;
	}
#ifdef SSHBBS
	...
	if (childpid == 0) {
		...
		dup2(0, 1);
		dup2(0, 2);
		setuserfile(editfile, "bbsnet");
		execl("bin/bbsnet", "bbsnet",
		      (cmd[0] == 'A') ? editfile : "etc/bbsnet.ini",
	...
	case '7':
		tmpu.userlevel |= PERM_DENYSIG;
		{
			...
			sprintf(content,
				...
				genbuf);
			system_mail_buf(content, strlen(content), tmpu.userid, "您被禁止使用签名档", currentuser->userid);
		}
		break;
	case 'g':
	case 'G':
...
static int finddf(int day, char *checkid)
{
	...
	sprintf(dirfile, MY_BBS_HOME"/bbstmpfs/tmp/checkid.%s.%d", checkid, uinfo.pid);
	fp = fopen(dirfile, "w");
	if (fp == NULL)
		return -1;
	if (starttime < 0)
	...
	while (fread(&board1, sizeof (board1), 1, fpb)) {
		...
		MMAP_TRY {
			...
			start = Search_Bin((struct fileheader *)mf.ptr, starttime, 0, total - 1);
			if (start < 0) {
				start = - (start + 1);
			}
...
s_checkid()
{
	...
	sprintf(buf, MY_BBS_HOME "/bbstmpfs/tmp/checkid.%s.%d",
		checkuser, uinfo.pid);
	mail_file(buf, currentuser->userid, "\"System Report\"", currentuser->userid);
	unlink(buf);
	prints("完毕");
	clrtoeol();
	pressreturn();
...
zsend_file(char *from, char *title)
{
	...
	name[78] = '\0';
	escape_filename(name);
	sprintf(name1, "yilubbs-%s-", currboard);
	strcat(name1, name);
	strcpy(name, name1);
	strcat(name1, ".TXT");
