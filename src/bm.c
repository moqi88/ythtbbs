...
static int deldeny(char *uident, int isglobal, int isanony, int ischange, char *msgbuf);
...
const static char *denyClass[] = {"版主", "站务", "讨论区总管", "分区站务", NULL};
...
int *limit;
{
	char buf[50], strtosave[256],*t1, *t2, *t3;
	char buf2[50];
	int day;
	time_t nowtime;
	char ans;
	...
	if(ischange && seek){
		if ((fp = fopen(genbuf, "r")) == NULL) {
	        ...
	        while (fgets(genbuf, 100, fp) != NULL){
			t1 = strtok(genbuf, " \t\n");
			t2 = strtok(NULL, " \t\n");
			t3 = strtok(NULL, "\t\n");
			
			if (t1 == NULL || t2 == NULL || t3 == NULL || strcmp(t1, uident))
				continue;
			else{
				prints("\n现有封禁理由及解封日期：%s  %s",t2, t3);
				break;
			}
			prints("没有记录？");
			pressreturn();
	...
	*dday = day;
	...
	if (ischange){
		if(isanony)
			deldeny(uident, 0, isanony, 0, genbuf);
		else
			deldeny(uident, isglobal, 0, 1,genbuf);
		}
	if (isglobal)
		strcpy(genbuf, "deny_users");
	else if (isanony)
...
static int
deldeny(uident, isglobal, isanony, ischange, msgbuf)
char *uident;
int isglobal;
int isanony;
int ischange;
char *msgbuf;
{
	...
	if(!ischange){
		buf[0] = 0;
		while (strlen(buf) < 4)
		        getdata(3, 0, "输入理由(至少两字): ", buf, 40, DOECHO, YEA);
		sprintf(msgbuf, "解封理由：\033[1;32m%s\033[m\n",buf);
	}
	if (isglobal)
...
deny_user()
{
	...
	if (!strcmp(currboard, "Penalty")) {
		if (!USERPERM(currentuser, PERM_SYSOP) && 
				!USERPERM(currentuser, PERM_ACCOUNTS)) {
			isglobal = 1;
			limit = 2;
		} else {
	...
	if (!strcmp(currboard, "IDmanage")) {
		if (!USERPERM(currentuser, PERM_SYSOP) &&
			...
			move(0, 0);
			clrtoeol();
			ans = askone(0, 0, "设定即将执行杀档的名单? (y/N)  ", "YN", 'N');
			if (ans == 'Y' || ans == 'y')
				setexecuteuser();
			return FULLUPDATE;
		}
...
//      ansimore(genbuf, YEA);
	while (1) {
		...
		if (ans == 'A') {
			...
			clrtoeol();
			if (uident[0] != '\0') {
				if (deldeny(uident, isglobal, 0, 0, msgbuf)) {
...
char *direct;
{
	...
	if (seek) {		/* 改变 or 解封 */
		ans = askone(1, 0, "(A)改变 (B)解封 " "[B]: ", "AB", 'B');
		if(ans == 'A'){
			if (askone(5, 0, "真的要改变封禁吗?[y/N]: ", "YN", 'N') == 'N')
				return -1;
			clear();
			if (addtodeny(user, msgbuf, 1, 0, isanony, &day, &limit) == 1) 
				deny_notice(CHANGEDENY, user, 0, isanony, msgbuf, day,limit);
			}
		else if(ans == 'B'){
			move(2, 0);
			if (askone(4, 0, "真的要解封么?[y/N]: ", "YN", 'N') == 'N')
				return -1;
			if (deldeny(user, 0, isanony, 0, msgbuf) == 1)
...
int limit;
{
	...
	case DENY:
		if (isglobal) {
			...
			sprintf(msgbuf,
				"%s %s 认为 %s 有 \033[1;32m%s\033[0m  行为\n"
	...
	case CHANGEDENY:
		sprintf(repbuf, "%s 改变封 %s 的时间或说明",
			currentuser->userid, isanony?"Anonymous":user);
		securityreport(repbuf, msgbuf);
		deliverreport(repbuf, msgbuf);
		if (!hideboard(currboard))
			penaltyreport(repbuf, msgbuf);
