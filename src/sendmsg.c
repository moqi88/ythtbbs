...
static int
isrobot (const char *msgstr) {
	struct msghead head;
	char filepath[STRLEN], tmpmsg[MAX_MSG_SIZE];
	int fd, count, i, j;
	time_t now = time(NULL);
	char id[64][IDLEN + 2];
	int diffusers = 0, samecontents = 0, total = 0, retv;
	int rate1, rate2;

	sethomefile(filepath, currentuser->userid, "msgindex");
	bzero(&id, sizeof(id));

	if ((fd = open(filepath, O_RDONLY, 0600)) < 0)
		return 0;

	count = (file_size(filepath) - 4) / sizeof(struct msghead);

	for (i = count - 1; i >= 0; i--) {
		lseek(fd, 4 + i * sizeof(struct msghead), SEEK_SET);
		read(fd, &head, sizeof(struct msghead));
		if (head.mode != 2 || !head.sent)
			continue;
		if (now - head.time > 120)	// 只统计两分钟
			break;
		total++;
		j = 0;
		retv = 0;
		while (id[j][0] && j < 64) {
			if (!strcasecmp(id[j], head.id)) {
				retv = 1;
				break;
			}
			j++;
		...
		if (!retv) {
			...
			diffusers++;
		load_msgtext(currentuser->userid, &head, tmpmsg);
		if (!strcmp(msgstr, tmpmsg))
			samecontents++;
	}
	...
	rate1 = 100 * diffusers / total;
	rate2 = 100 * samecontents / total;
	
	if (rate1 > 80 || rate2 > 50)
		do_delay(1);
	return 0;
}
	    ...
	    const char *msgstr, int mode, char *msgerr)
{
	...
	*msgerr = 0;
	...
	if (mode == 2 && time(NULL) - lastsend < 2) {
		isrobot(msgstr);
	}
	if (uin == NULL) {
		offline_msg = 1;
